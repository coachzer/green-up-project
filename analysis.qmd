...
combined_data <- bind_rows(without_company_data, g.2019.data, g.2020.data, g.2021.data, g.2022.data, g.2023.data)
...
# initial stock to csv
write_csv(initial_stock, "Interface/Visualization/data/initial_stock.csv")
...
p1 <- rates_daily |>
  ...
  plot_ly(x = ~statistical_region, 
          y = ~total_annual, 
          type = 'bar',
          color = ~statistical_region,
          colors = region_colors) |>
  layout(title = "Total Annual Waste by Region",
         xaxis = list(title = "Region"),
         yaxis = list(title = "Annual Tonnes"))

# Waste by code (top 10)
p2 <- rates_daily |>
  group_by(waste_code) |>
  summarise(total_annual = sum(annual_tonnes)) |>
  top_n(10, total_annual) |>
  plot_ly(x = ~reorder(waste_code, total_annual), y = ~total_annual, type = 'bar') |>
  layout(title = "Top 10 Waste Codes by Total Annual Tonnes",
         xaxis = list(title = "Waste Code"),
         yaxis = list(title = "Annual Tonnes"))
...
p3 <- rates_daily |>
  ...
  summarise(total_annual = sum(annual_tonnes)) |>
  group_by(statistical_region) |>
  top_n(5, total_annual) |>
  plot_ly(x = ~statistical_region, 
          y = ~total_annual, 
          color = ~waste_code, 
          type = 'bar') |>
  layout(title = "Top 5 Waste Codes by Region",
         xaxis = list(title = "Region"),
         yaxis = list(title = "Annual Tonnes"),
         barmode = 'stack')

heatmap_data <- rates_daily |>
  select(statistical_region, waste_code, annual_tonnes) |>
  pivot_wider(names_from = waste_code, values_from = annual_tonnes, values_fill = 0)

p4 <- rates_daily |>
  plot_ly(x = ~daily_tonnes, y = ~annual_tonnes,
          color = ~statistical_region,
          colors = region_colors,
          size = ~annual_tonnes,
          text = ~paste("Region:", statistical_region, "<br>Code:", waste_code),
          hovertemplate = "%{text}<br>Daily: %{x}<br>Annual: %{y}<extra></extra>") |>
  add_markers() |>
  layout(title = "Daily vs Annual Waste Generation",
         xaxis = list(title = "Daily Tonnes"),
         yaxis = list(title = "Annual Tonnes"))

treemap_data <- rates_daily |>
  # First add region totals as parent nodes
  group_by(statistical_region) |>
  summarise(total_annual = sum(annual_tonnes)) |>
  mutate(
  ...
  # Then add individual waste codes as children
  bind_rows(
    rates_daily |>
      mutate(
...
# Plot 6: Waste treated by the original generator
waste_treated_by_generator <- combined_data |>
  group_by(type_of_waste, year) |>
  summarize(total_treated = sum(waste_treated_by_original_generator, na.rm = TRUE)) |>
  ungroup()

t6 <- ggplot(
  waste_treated_by_generator, 
  aes(x = year, y = total_treated, color = type_of_waste)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Total Wood Waste Treated by the Original Generator by Year",
    x = "Year",
    y = "Total Waste Treated (tons)"
  ) +
  my_theme +
  scale_x_continuous(breaks = unique(waste_treated_by_generator$year))
...
heatmap_plot <- ggplot(category_type_summary, aes(x = type_of_waste, y = waste_category, fill = total_collected)) +
  geom_tile() +
  labs(
    title = "Waste Collection: Category vs Type Heatmap",
    x = "Type of Waste", y = "Waste Category",
...
ggplotly(heatmap_plot, width = 1000, height = 600)
...
```{r 2023_municipal_waste_collected}
...
t10 <- ggplot(df_long_municipal, aes(x = year, y = total_collected, color = type_of_waste, group = name_of_municipality)) +
  geom_line() +
  geom_point() +
  facet_wrap(~statistical_region, scales = "free_y") +
  labs(
    title = "Total Waste Collected by Municipality and Region (2018-2023)",
    x = "Year",
...
t11 <- ggplot(df_top_municipal, aes(x = year, y = total_collected, color = name_of_municipality, group = name_of_municipality)) +
  ...
  geom_point() +
  labs(
    title = "Total Waste Collected by Top 5 Municipalities (2018-2023)",
    x = "Year",
    y = "Total Waste Collected (in tons)",
    color = "Municipality"
  ) +
...
# combine data
combined_collected_data <- bind_rows(waste.collected.2018.data, waste.collected.2019.data, waste.collected.2020.data, waste.collected.2021.data, waste.collected.2022.data, waste.collected.2023.data) |> dplyr::select(year, statistical_region, name_of_municipality, type_of_waste, waste_category, waste_by_municipality)
...
# Aggregate data by statistical region and year
regional_trends <- combined_collected_data |>
  group_by(year, statistical_region, waste_category) |>
  summarise(total_waste = sum(waste_by_municipality, na.rm = TRUE), .groups = 'drop')

regional_trends_plot <- ggplot(regional_trends, aes(x = year, y = total_waste, color = waste_category)) +
  geom_line(size = 1.2, alpha = 0.8) +
  geom_point(size = 2, alpha = 0.9) +
  facet_wrap(~statistical_region, scales = "free_y", ncol = 2) +
  labs(
    title = "Waste Collection Trends by Statistical Region (2018-2023)",
    subtitle = "Showing waste category breakdown across regions over time",
    x = "Year",
    y = "Total Waste Collected (tons)",
    color = "Waste Category"
  ) +
  my_theme +
  theme(
...
ggplotly(regional_trends_plot, width = 1000, height = 1000)
...
#  [4] "type_of_waste"                  
#  [5] "waste_category"                 
#  [6] "total_waste_given_away"         
#  [7] "waste_handed_to_collectors_RS"  
#  [8] "waste_delivered_to_operators_RS"
#  [9] "waste_sent_to_EU"               
# [10] "waste_sent_to_non_EU"  
...
aggregated_data <- combined_treatment_data |>
  ...
  summarize(total_waste_processing = sum(waste_for_processing, na.rm = TRUE))

# Create the ggplot visualization
p1 <- ggplot(aggregated_data, aes(x = year, y = total_waste_processing, color = waste_category, group = type_of_waste)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
  labs(
...
aggregated_data_category <- combined_treatment_data |>
  # filter out statistical_region = "NEOPREDELJENO"
  filter(statistical_region != "NEOPREDELJENO") |>
  group_by(year, waste_category) |>
  summarize(total_waste_processing = sum(waste_for_processing, na.rm = TRUE))
...
# Create the ggplot visualization
p1 <- ggplot(aggregated_data_category, aes(x = year, y = total_waste_processing, color = waste_category, group = waste_category)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
  labs(
    title = "Waste Treatment Over Time by Waste Category",
    x = "Year",
    y = "Total Waste Entering Treatment Process",
    color = "Waste Category"
  ) +
