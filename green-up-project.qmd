...
waste_plot_data <- generation.23 |>
  ...
  summarise(total_generated = sum(waste_generated, na.rm = TRUE)) |>
  mutate(
    percentage = total_generated/sum(total_generated),
    display_label = paste0(comma(total_generated), " (", percent(percentage, accuracy = 0.01), ")")
  ) |> 
...
waste_collected.19 <- waste_recovery_19.sheet2 |>
  ...
  mutate(
    ...
    waste_received_from_other_producer = case_when(
      received_from == "other producer in RS" ~ total_waste_received,
      TRUE ~ 0
    ),
  ...
  summarise(
    ...
    waste_received_from_other_producer = sum(waste_received_from_other_producer) / 1000,
