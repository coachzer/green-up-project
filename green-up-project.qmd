...
# 17 02 01 (wood from construction)
# 20 01 38 (wood not specified under other categories)
codes <- c("15 01 03", "15 01 01", "17 02 01", "20 01 38", "20 01 37")

# 02 WASTE FROM AGRICULTURE, HORTICULTURE, AQUACULTURE, FORESTRY, HUNTING AND FISHING, FOOD PREPARATION AND PROCESSING

# 02 01 Waste from agriculture, horticulture, aquaculture, forestry, hunting and fishing
# 02 01 07 Waste from forestry

# 03 WASTE FROM WOOD PROCESSING AND PRODUCTION OF PANELS AND FURNITURE, PULP, PAPER AND CARDBOARD

# 03 01 Waste from wood processing and production of panels and furniture
# 03 01 99 Other similar wastes
...
# 19 WASTE FROM WASTE MANAGEMENT FACILITIES, WASTEWATER TREATMENT PLANTS AND WATER TREATMENT PLANTS

# 19 12 Waste from mechanical waste treatment (e.g. sorting, crushing, compacting, pelleting) not elsewhere specified
# 19 12 01 Paper and cardboard
# 19 12 06* Wood containing hazardous substances
# 19 12 07 Wood not specified under 19 12 06
...
new_codes <- c("03 03 01", "15 01 03", "15 01 01", "17 02 01", "20 01 38", "02 01 07", "03 01 99", "03 03 07", "19 12 01", "19 12 06", "19 12 07", "20 01 37", "20 03 07")
...
rename_waste_names <- function(df, column_name = "type_of_waste") {
  ...
  df |>
    mutate(!!column_name := case_when(
      ...
      !!column_name == "Odpadna lubje in les" ~ "Waste Bark and Wood",
      !!column_name == "Kovine" ~ "Metals",
      !!column_name == "Kosovni odpadki" ~ "Bulky Waste",
      ...
      !!column_name == "Les, ki ni naveden v 19 12 06" ~ "Wood, not specified in 19 12 06",
      ...
      !!column_name == "Mehansko ločeni rejekti iz razpuščanja odpadnega papirja ter kartona in lepenke" ~ "Mechanically Separated Rejects from Waste Paper and Cardboard Pulping",
      !!column_name == "Odpadki iz gozdarstva" ~ "Forestry Waste",
...
generation.23 <- rename_waste_names(generation.23)
...
#### Looking at new waste codes

```{r new_waste_codes}

data_with_old_codes <- read_csv("2023 data\\2023 filtered\\generation\\old_codes.csv")

data_with_new_codes <- read_csv("2023 data\\2023 filtered\\generation\\new_codes.csv")

newly_added_codes <- setdiff(data_with_new_codes$waste_code, data_with_old_codes$waste_code)
cat("Newly added waste codes:", newly_added_codes, "\n")

# Sum waste generated ONLY for newly added codes
new_codes_waste <- data_with_new_codes %>%
  filter(waste_code %in% newly_added_codes) %>%
  summarise(total_new_waste = sum(generated_in_the_year, na.rm = TRUE)) %>%
  pull(total_new_waste)

cat("Total waste from newly added codes:", new_codes_waste, "\n")

# Compare to total waste in the new dataset
total_new_dataset <- sum(data_with_new_codes$generated_in_the_year, na.rm = TRUE)
cat("Percentage contribution of new codes:", round(new_codes_waste / total_new_dataset * 100, 2), "%\n")

new_codes_breakdown <- data_with_new_codes %>%
  filter(waste_code %in% newly_added_codes) %>%
  group_by(waste_code, type_of_waste) %>%
  summarise(waste_generated = sum(generated_in_the_year, na.rm = TRUE)) %>%
  arrange(desc(waste_generated))

# Print top 5 contributors
print("Top new waste contributors:")
print(head(new_codes_breakdown, 5))

# Plot 1: New vs. Existing waste composition
composition_data <- data.frame(
  category = c("Existing Codes", "Newly Added Codes"),
  waste = c(total_new_dataset - new_codes_waste, new_codes_waste)
)

ggplot(composition_data, aes(x = "", y = waste, fill = category)) +
  geom_col(width = 0.7) +
  geom_text(
    aes(label = paste0(waste, " tons (", round(waste / total_new_dataset * 100, 1), "%)")),
    position = position_stack(vjust = 0.5),
    color = "white",
    size = 5
  ) +
  labs(
    title = "Total Waste: Contribution of New vs. Existing Codes",
    x = NULL,
...
# Plot 2: New waste codes
ggplot(new_codes_breakdown, aes(x = reorder(waste_code, -waste_generated), y = waste_generated)) +
  geom_col(fill = "steelblue", alpha = 0.8, width = 0.7) +  # Slightly transparent and narrower bars
  geom_text(
    aes(label = scales::comma(waste_generated)),  # Formats numbers with commas (e.g., 8,200)
    vjust = -0.5,                                 # Positions labels above bars
    size = 3.5,                                   # Smaller font for minimalism
    color = "black",                              # High contrast
    fontface = "bold"
  ) +
  labs(
    title = "New Waste Contributors",
    x = "Waste Code",
    y = "Waste Generated (tons)"
  ) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +  # Adds headroom for labels
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 45, hjust = 1)  # Rotates x-labels for readability
  )
...
collected.23 <- rename_waste_names(collected.23)
...
collected_center.23 <- rename_waste_names(collected_center.23)

collected_center.23 <- collected_center.23 |>
  group_by(year, statistical_region, collection_centre, type_of_waste) |>
...
colnames(collected_additional.23)
...
collected_additional.23 <- rename_waste_names(collected_additional.23)
...
collected_additional.23 <- collected_additional.23 |>
  group_by(year, type_of_waste, waste_collected_public_service) |>
  summarise(
    total_waste_collected = sum(total_waste_collected) / 1000,
...
colnames(collected_municipality.23)
...
collected_municipality.23 <- rename_waste_names(collected_municipality.23)
...
collected_municipality.23 <- collected_municipality.23 |>
  group_by(year, statistical_region, type_of_waste, name_of_municipality, waste_collected_public_service) |>
...
colnames(collected_management.23)
...
collected_management.23 <- rename_waste_names(collected_management.23)
...
collected_management.23 <- collected_management.23 |>
  ...
  summarise(
    ...
    # waste_collected_to_collectors_warehouse = sum(waste_collected_to_collectors_warehouse) / 1000)
  ) |>
  ungroup()
...
colnames(household_composting.23)
...
treatment.23 <- rename_waste_names(treatment.23, "type_of_waste_inlet")
treatment.23 <- rename_waste_names(treatment.23, "type_of_waste_outlet")
...
treatment.23 <- treatment.23 |>
  group_by(year, statistical_region, type_of_waste_inlet, type_of_waste_outlet, substance_or_object) |>
...
colnames(treatment_received.23)
...
treatment_received.23 <- rename_waste_names(treatment_received.23)
...
waste_treated.23 <- rename_waste_names(waste_treated.23)
...
management.23 <- rename_waste_names(management.23, "input_waste_name")
management.23 <- rename_waste_names(management.23, "output_waste_name")

management.23 <- management.23 |>
  group_by(year, statistical_region, input_waste_name, treatment_operation, output_waste_name, further_treatment_envisaged) |>
  summarise(
...
colnames(management_products.23)
...
colnames(landfill_management.23)
...
colnames(disposed_municipality.23)
...
processed_sheets[[1]] <- pre_collection.22 |> select(-waste_code)
...
management.22 <- management.22 |> select(-waste_code)

processed_sheets[[5]] <- management.22 |> select(year, statistical_region, type_of_waste, total_waste_given_away, waste_handed_to_collectors_RS, waste_delivered_to_operators_RS, waste_sent_to_EU, waste_sent_to_non_EU)
...
processed_sheets[[1]] <- treatment.22 |> select(-waste_code)
...
management.22 <- management.22 |>
  group_by(year, statistical_region, input_waste_name, treatment_operation, output_waste_name) |>
  summarize(
...
processed_sheets[[5]] <- management_no_waste.22 |> select(-input_waste_code)
...
processed_sheets[[7]] <- mass_difference.22 |> select(-input_waste_code)
...
processed_sheets[[1]] <- rename_waste_names(processed_sheets[[1]])
...
waste_storage.19 <- waste_storage.19 |>
  mutate(
    type_of_waste = case_when(
      waste_code == "03 01 01" ~ "Waste Bark and Cork",
      waste_code == "03 01 05" ~ "Sawdust/Chips/Wood, not specified in 03 01 04",
      waste_code == "03 03 01" ~ "Waste Bark and Wood",
      waste_code == "15 01 01" ~ "Paper/Cardboard Packaging",
      waste_code == "15 01 03" ~ "Wooden packaging",
      waste_code == "17 02 01" ~ "Wood",
      waste_code == "20 01 38" ~ "Wood, not specified in 20 01 37",
      waste_code == "03 01 99" ~ "Miscellaneous Waste"
    )
  )
...
## Rename
colnames(waste_collected_municipality.19)
...
## Rename
colnames(municipal_waste_origin_19)
...
## Rename
colnames(waste_treatment_process.19)
...
## Rename
colnames(municipal_waste_treatment.19)
...
## Rename
colnames(waste_volumes_landfill.19)
...
## Rename
colnames(waste_collection_18.by_codes1)
...
## Rename
colnames(waste_collection_18.by_codes2)
...
## Rename
colnames(waste_collection_18.by_codes3)
...
## Rename
colnames(waste_collection_18.by_codes4)
...
## Rename
colnames(waste_collection_18.by_codes5)
...
## Rename
colnames(waste_recovery_18.by_codes1)
...
## Rename
colnames(waste_recovery_18.by_codes2)
...
## Rename
colnames(waste_recovery_18.by_codes3)
...
## Rename
colnames(waste_recovery_18.by_codes4)
...
## Rename
colnames(waste_recovery_18.by_codes5)
...
## Rename
colnames(waste_recovery_18.by_codes6)
...
## Rename
colnames(waste_recovery_18.sheet7)
...
## Rename
colnames(waste_recovery_18.by_codes8)
...
## Rename
colnames(waste_recovery_18.sheet9)
...
## Rename
colnames(waste_generation_17.by_codes1)
...
## Rename
colnames(waste_generation_17.by_codes2)
...
waste_generation_17.by_codes2 <- waste_generation_17.by_codes2 |>
  select(-waste_code, -treatment_process)
...
## Rename
colnames(waste_collection_17.by_codes1)
...
## Rename
colnames(waste_collection_17.by_codes2)
...
## Rename
colnames(waste_collection_17.by_codes3)
...
## Rename
colnames(waste_recovery_disposal_17.by_codes1)
...
## Rename
colnames(waste_recovery_disposal_17.by_codes2)
...
## Rename
colnames(waste_recovery_disposal_17.by_codes3)
...
## Rename
colnames(waste_recovery_disposal_17.by_codes4)
...
## Rename
colnames(waste_recovery_disposal_17.by_codes5)
...
## Rename
colnames(waste_recovery_disposal_17)
...
## Rename
colnames(waste_recovery_disposal_17.by_codes7)
...
waste_recovery_disposal_17.by_codes7 <- waste_recovery_disposal_17.by_codes7 |>
  group_by(name_of_company, landfill, type_of_waste) |>
  summarise(
...
## Rename
colnames(waste_generation_16.by_codes1)
...
waste_generation_16.by_codes1 <- waste_generation_16.by_codes1 |>
  mutate(
    type_of_waste = case_when(
      waste_code == "02 01 07" ~ "Forestry Waste",
      waste_code == "03 01 01" ~ "Waste Bark and Cork",
      waste_code == "03 01 05" ~ "Sawdust/Chips/Wood, not specified in 03 01 04",
      waste_code == "03 03 01" ~ "Waste Bark and Wood",
      waste_code == "03 03 07" ~ "Mech_Sep_Rejects_Pulp", # Mechanically separated rejects from pulping of waste paper and cardboard
      waste_code == "15 01 01" ~ "Paper/Cardboard Packaging",
      waste_code == "15 01 03" ~ "Wooden packaging",
      waste_code == "17 02 01" ~ "Wood",
      waste_code == "19 12 01" ~ "Paper and Cardboard",
      waste_code == "19 12 07" ~ "Wood not specified under 19 12 06",
      waste_code == "20 03 07" ~ "Bulky Waste",
      waste_code == "20 01 38" ~ "Wood, not specified in 20 01 37",
      waste_code == "03 01 99" ~ "Miscellaneous Waste"
    )
  )
...
waste_generation_16.by_codes1 <- waste_generation_16.by_codes1 |>
  select(year, statistical_region, type_of_waste, total_waste_generated, waste_stored_start_year, waste_generated, waste_stored_end_year)
```

#### New codes

```{r}
old_codes_data_2016 <- waste_generation_16.by_codes1
new_codes_data_2016 <- read_csv("2016 data\\2016 filtered\\generation\\new_codes_16.csv")

newly_added_types <- setdiff(new_codes_data_2016$type_of_waste, old_codes_data_2016$type_of_waste)
cat("Newly added waste types:", newly_added_types, "\n")

# Sum waste generated ONLY for newly added codes
new_types_waste <- new_codes_data_2016 %>%
  filter(type_of_waste %in% newly_added_types) %>%
  summarise(total_new_waste = sum(total_waste_generated, na.rm = TRUE)) %>%
  pull(total_new_waste)

cat("Total waste from newly added codes:", new_types_waste, "\n")

# Compare to total waste in the new dataset
total_new_dataset <- sum(new_codes_data_2016$total_waste_generated, na.rm = TRUE)
cat("Percentage contribution of new types:", round(new_types_waste / total_new_dataset * 100, 2), "%\n")

new_waste_types <- setdiff(
  unique(new_codes_data_2016$type_of_waste),
  unique(old_codes_data_2016$type_of_waste)
)

# Then analyze these new types
new_types_breakdown <- new_codes_data_2016 %>%
  filter(type_of_waste %in% new_waste_types) %>%
  group_by(type_of_waste) %>%  # Group by type instead of code
  summarise(
    waste_generated = sum(total_waste_generated, na.rm = TRUE),
    .groups = 'drop'
  ) %>%
  arrange(desc(waste_generated))  # Sort descending by volume

print(new_types_breakdown)

# Plot 1: New vs. Existing waste composition
composition_data <- data.frame(
  category = c("Existing Codes", "Newly Added Codes"),
  waste = c(total_new_dataset - new_types_waste, new_types_waste)
)
...
waste_code_labels <- c(
  ...
  "Mech_Sep_Rejects_Pulp" = "03 03 07",
  "Bulky Waste" = "20 03 07",
  "Paper and Cardboard" = "19 12 01",
  "Wood not specified under 19 12 06" = "19 12 07",
  "Forestry Waste" = "02 01 07"
)

# Apply the labels using scale_x_discrete()
ggplot(new_types_breakdown, aes(x = reorder(type_of_waste, -waste_generated), y = waste_generated)) +
  geom_col(fill = "steelblue", alpha = 0.8, width = 0.7) +
  geom_text(
    aes(label = scales::comma(waste_generated)),
    vjust = -0.5,
    size = 3.5,
    color = "black",
    fontface = "bold"
  ...
  scale_x_discrete(labels = waste_code_labels) +  # Replace x-axis labels with waste codes
...
## Rename
colnames(waste_generation_16.by_codes2)
...
## Rename
colnames(waste_collection_16.by_codes1)
...
## Rename
colnames(waste_collection_16.by_codes2)
...
## Rename
colnames(waste_collection_16.by_codes3)
...
## Rename
colnames(waste_recovery_disposal_16.by_codes1)
...
## Rename
colnames(waste_recovery_disposal_16.by_codes2)
...
# create fromm collected_from column
waste_recovery_disposal_16.by_codes2 <- waste_recovery_disposal_16.by_codes2 |>
  mutate(
...
## Rename
colnames(waste_recovery_disposal_16.by_codes3)
...
## Rename
colnames(waste_recovery_disposal_16.by_codes4)
