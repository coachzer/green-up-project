...
generation.21 |> filter(region == "GORENJSKA")
...
find_municipality_with_most_waste <- function(region_municipalities, waste_df) {
  ...
  municipality_with_most_waste <- aggregate(`Komunalni odpadki po občini izvora kg` ~ `Naziv občine`, data = region_data, FUN = sum)
  municipality_with_most_waste <- municipality_with_most_waste[which.max(municipality_with_most_waste$`Komunalni odpadki po občini izvora kg`), ]
...
list_of_waste_by_municipality <- function(region_municipalities, waste_df) {
  ...
  total_waste_by_municipality <- aggregate(`Komunalni odpadki po občini izvora kg` ~ `Naziv občine`, data = region_data, FUN = sum)
  # Sort municipalities by waste amount in descending order
  sorted_municipalities <- total_waste_by_municipality[order(-total_waste_by_municipality$`Komunalni odpadki po občini izvora kg`), ]
...
list_of_waste_by_municipality(OBALNOKRAŠKA, wc_by_origin)
