...
packages <- c("tidyverse", "gt", "gtExtras", "readxl", "ggplot2", 
...
2.  Cleaned up all the years and ready for combining data per type (generation, collection or treatment)
...
generation.22 <- generation.22 |> 
  ...
  summarise(total_waste_generated_stored = sum(total_waste_generated_stored) / 1000,
...
pre_collection.22 <- pre_collection.22 |> 
  mutate(previous_stored_start_year = as.numeric(previous_stored_start_year) / 1000,
...
collected.22 <- collected.22 |> 
  group_by(year, statistical_region, type_of_waste) |>
  mutate(waste_from_producers_no_record = as.numeric(waste_from_producers_no_record) / 1000,
...
collected_mun.22 <- collected_mun.22 |> 
  mutate(total_mun_waste_collected = as.numeric(total_mun_waste_collected) / 1000,
...
management.22 <- management.22 |> 
  group_by(year, statistical_region, type_of_waste) |>
  mutate(waste_handed_to_collectors_RS = as.numeric(waste_handed_to_collectors_RS) / 1000,
...
waste_received.22 <- waste_received.22 |> 
  group_by(year, statistical_region, type_of_waste) |>
  mutate(total_waste_received = as.numeric(total_waste_received) / 1000,
         untreated_waste_from_storage_start_year = as.numeric(untreated_waste_from_storage_start_year) / 1000,
         waste_received_own_waste = as.numeric(waste_received_own_waste) / 1000,
         waste_received_from_generators = as.numeric(waste_received_from_generators) / 1000,
         waste_received_from_collectors = as.numeric(waste_received_from_collectors) / 1000,
         waste_received_from_treatment = as.numeric(waste_received_from_treatment) / 1000,
         waste_received_from_EU = as.numeric(waste_received_from_EU) / 1000,
         waste_received_from_non_EU = as.numeric(waste_received_from_non_EU) / 1000) |>
  ungroup()

processed_sheets[[2]] <- waste_received.22 |> select(-waste_code)
...
waste_treated.22 <- waste_treated.22 |> 
  group_by(year, statistical_region, type_of_waste, description_of_treatment) |>
  mutate(waste_quantity_per_treatment = as.numeric(waste_quantity_per_treatment) / 1000) |>
  ungroup()

processed_sheets[[3]] <- waste_treated.22 |> select(-waste_code)
...
management.22 <- management.22 |> 
  mutate(waste_sent_to_self_storage_end_year = as.numeric(waste_sent_to_self_storage_end_year) / 1000,
         waste_sent_to_collector_SI = as.numeric(waste_sent_to_collector_SI) / 1000,
         waste_sent_to_treatment_operator = as.numeric(waste_sent_to_treatment_operator) / 1000,
         waste_sent_to_EU_member_states = as.numeric(waste_sent_to_EU_member_states) / 1000,
         waste_sent_to_non_EU_countries = as.numeric(waste_sent_to_non_EU_countries) / 1000,
         waste_sent_to_no_further_management = as.numeric(waste_sent_to_no_further_management) / 1000) 

processed_sheets[[4]] <- management.22 |> select(-treatment_input_waste_received_code, -treatment_outlet_delivered_waste_code)
...
management_no_waste.22 <- management_no_waste.22 |> 
  mutate(storage_operator_end_year = as.numeric(storage_operator_end_year) / 1000,
...
processed_sheets[[5]] <- management_no_waste.22 |> select(-waste_code)
...
processed_sheets[[7]] <- mass_difference.22 |> select(-waste_code)
...
compost_digestate.22 <- compost_digestate.22 |> 
  ...
  summarise(quantity_produced_kg = sum(quantity_produced_kg) / 1000,
...
compost_digestate.22 <- compost_digestate.22 |> 
  rename(quantity_produced_t = quantity_produced_kg,
...
processed_sheets[[1]] <- processed_sheets[[1]] |> 
  group_by(year, statistical_region, type_of_waste) |>
  summarise(previous_stored_start_year = sum(previous_stored_start_year) / 1000,
...
processed_sheets[[2]] <- processed_sheets[[2]] |> 
  group_by(year, statistical_region, type_of_waste) |>
  summarise(waste_from_producers_no_record = sum(waste_from_producers_no_record) / 1000,
...
processed_sheets[[3]] <- processed_sheets[[3]] |> 
  group_by(year, type_of_waste) |>
  summarise(total_municipal_waste_collected = sum(total_municipal_waste_collected) / 1000,
...
processed_sheets[[5]] <- processed_sheets[[5]] |> 
  group_by(year, statistical_region, type_of_waste) |>
  summarise(waste_handed_to_collectors_RS = sum(waste_handed_to_collectors_RS) / 1000,
...
waste_received.21 <- waste_received.21 |> 
  select(-waste_code) |> 
  mutate(
    waste_received_own_waste = waste_received_own_waste / 1000,
    waste_received_from_producers_RS = waste_received_from_producers_RS / 1000,
    ...
    waste_received_from_EU = waste_received_from_EU / 1000,
    waste_received_from_non_EU = waste_received_from_non_EU / 1000,
...
treating_waste.21 <- treating_waste.21 |> 
  select(-waste_code) |>
  mutate(
    waste_entering_treatment_process = waste_entering_treatment_process / 1000
  )
...
management_waste.21 <- management_waste.21 |> 
  select(-treatment_input_waste_received_code, -treatment_outlet_delivered_waste_code) |>
  mutate(
...
management_waste.21a <- management_waste.21a |> 
  select(-waste_code) |>
  mutate(
    delivered_to_storage_or_other_persons_RS = delivered_to_storage_or_other_persons_RS / 1000,
    shipped_to_EU = shipped_to_EU / 1000,
    shipped_to_non_EU = shipped_to_non_EU / 1000
  )
...
waste_treatment.21 <- waste_treatment.21 |>
  select(-name_of_municipality) |>
  mutate(
    waste_collected_by_municipality = waste_collected_by_municipality / 1000
  )
...
landfill_waste.21 <- landfill_waste.21 |> 
  ...
  mutate(
    total_waste_received = total_waste_received / 1000,
    waste_without_assessment = waste_without_assessment / 1000,
    disposed_waste_total = disposed_waste_total / 1000,
    disposed_waste_hazardous = disposed_waste_hazardous / 1000,
    disposed_waste_non_hazardous = disposed_waste_non_hazardous / 1000,
    disposed_waste_inert = disposed_waste_inert / 1000,
    disposed_waste_for_construction = disposed_waste_for_construction / 1000,
    disposed_waste_for_covering = disposed_waste_for_covering / 1000
  )
...
mass_difference.20 <- mass_difference.20 |> 
  mutate(
    mass_change = as.numeric(mass_change) / 1000
  )
...
landfill_waste.20 <- landfill_waste.20 |> 
  mutate(
    total_waste_received = as.numeric(total_waste_received) / 1000,
    disposed_waste_total = as.numeric(disposed_waste_total) / 1000,
    disposed_waste_non_hazardous = as.numeric(disposed_waste_non_hazardous) / 1000
  )
...
waste_storage.19 <- waste_storage.19 |>
  ...
  summarise(waste_stored_start_year = sum(waste_stored_start_year) / 1000, 
...
waste_collected_municipality.19 <- waste_collected_municipality.19 |>
  mutate(
    ...
    municipal_waste_collected = municipal_waste_collected / 1000,
    waste_collected_from_activities = waste_collected_from_activities / 1000
  )
...
waste_handling_19.sheet5 <- waste_handling_19
...
waste_recovery_19.by_codes3 <- waste_treatment.19 |> select(year, statistical_region, type_of_waste, treatment_process, treatment_after_8, total_waste_for_processing)
```
...
waste_treatment_process.19 <- waste_treatment_process.19 |> 
  ...
  rename(
    ...
    statistical_region = `STATISTIČNA REGIJA`,
    waste_code = `waste_code`,
    waste_recovered_and_given_away
 = `PO OBDELAVI NASTALI ODPADKI IN ODPADKI ODDANI DRUGIM BREZ OBDELAVE  (kg)`,
    delivered_to = `ODDANO KOMU`,
...
waste_recovery_19.sheet8 <- waste_volumes_landfill.19
```
...
## if NA
waste_recovery_18.by_codes8[is.na(waste_recovery_18.by_codes8)] <- 0
```
...
waste_collection_17.by_codes2 <- waste_collection_17.by_codes2 |> 
  select(year, statistical_region, type_of_waste, total_waste_collected, waste_from_producers_no_record, waste_from_producers_with_record, waste_from_collectors_RS, waste_from_processors_RS)
...
waste_collection_17.by_codes3 <- waste_collection_17.by_codes3 |>
  select(year, statistical_region, type_of_waste, total_waste_given_away, waste_given_to_collector_RS, waste_given_to_processing_operator_RS, waste_given_to_other_EU_country, waste_given_to_other_country_outside_EU)
...
waste_recovery_disposal_17.by_codes2 <- waste_recovery_disposal_17.by_codes2 |>
  group_by(statistical_region, type_of_waste, received_from) |>
  summarise(
...
waste_recovery_disposal_17.by_codes3 <- waste_recovery_disposal_17.by_codes3 |>
  select(year, statistical_region, type_of_waste, total_waste_given_away, treatment_process, treatment_according_to_8th_article)
...
# List
waiste_generation.16 <- list(
  waste_generation_16.by_codes1,
  waste_generation_16.by_codes2
)

names(waiste_generation.16) <- sheet_names

for (sheet_name in sheet_names) {
  write_excel_csv(waiste_generation.16[[sheet_name]], paste0("2016 data\\2016 filtered\\generation\\", tolower(gsub(" ", "_", sheet_name)), "_16.csv"))
}
...
waste_recovery_disposal_16.by_codes3 <- waste_recovery_disposal_16.by_codes3 |>
  select(year, statistical_region, type_of_waste, total_waste_given_away, treatment_process)
