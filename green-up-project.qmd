...
# 03 03 08 - Waste from sorting of paper and cardboard for recycling
...
process_sheet <- function(path, sheet_index, n_max = 2, skip = 7) {
  ...
  header <- read_xlsx(path, skip = skip, sheet = sheet_index, col_names = FALSE, n_max = n_max)
  ...
  if (num_header_rows == 2) {
    ...
    # Read the sheet data
    sheet_data <- read_xlsx(path, skip = 7 + num_header_rows, sheet = sheet_index, col_names = FALSE)
...
# Function to filter by codes (for 2018, 2017...)
filter_codes <- function(.data, code = waste_code, codes_vec = NULL, include_pattern = "^03 01 [0-9]", exclude_pattern = "03 01 04") {
  # decide which codes to use
  codes_to_use <- if (is.null(codes_vec)) get("codes", envir = parent.frame()) else codes_vec
  
  code_quo <- enquo(code)
  
  .data |>
    filter(
      (!!code_quo) %in% codes_to_use |
      (str_detect(!!code_quo, include_pattern) & 
       !str_detect(!!code_quo, exclude_pattern))
    )
}
...
categorize_waste <- function(data, waste_code_col = "waste_code") {
  ...
  # Check if waste_category exists and find next available suffix
  waste_cat_cols <- names(data)[str_detect(names(data), "^waste_category(_\\d+)?$")]
  
  if (length(waste_cat_cols) > 0) {
    # Extract existing numbers
    nums <- str_extract(waste_cat_cols, "\\d+") |> 
      as.integer() |> 
      suppressWarnings()
    ...
    new_waste_cat_col <- paste0("waste_category_", next_num)
  } else {
    new_waste_cat_col <- "waste_category"
  }
  
  data |>
    mutate(
      type_of_waste = case_when(
        # 01 - Wastes from prospecting, mining, quarrying, physical and chemical processing of minerals
        # 01 04 - Wastes from physical and chemical processing of non-metallic minerals
        .data[[waste_code_col]] == "01 04 13" ~ "Waste from Cutting and Sawing of Stone (01 04 13)",
        
        # 02 - Agricultural, horticultural, aquacultural, forestry, hunting, and fishing waste
        # 02 01 - Waste from agriculture, horticulture, aquaculture, forestry, hunting and fishing
        .data[[waste_code_col]] == "02 01 03" ~ "Waste Plant Tissues (02 01 03)",
        .data[[waste_code_col]] == "02 01 06" ~ "Animal Excreta and Manure (02 01 06)",
        .data[[waste_code_col]] == "02 01 07" ~ "Forestry Waste (02 01 07)",
        
        # 02 02 - Waste from the preparation and processing of meat, fish and other foodstuffs of animal origin
        .data[[waste_code_col]] == "02 02 01" ~ "Washing and Cleaning Sludges (02 02 01)",
        .data[[waste_code_col]] == "02 02 04" ~ "Sludges from On-site Wastewater Treatment (02 02 04)",
        
        # 02 03 - Wastes from the preparation and processing of fruit, vegetables, cereals, edible oils, cocoa, coffee, tea and tobacco; from preserving; from the production of yeast and yeast extract, from the preparation and fermentation of molasses
        .data[[waste_code_col]] == "02 03 01" ~ "Peelings, Pulp and Other Residues from Fruit and Vegetable Processing (02 03 01)",
        .data[[waste_code_col]] == "02 03 04" ~ "Materials Unsuitable for Use or Processing (02 03 04)",
        .data[[waste_code_col]] == "02 03 05" ~ "Sludges from On-site Wastewater Treatment (02 03 05)",
        
        # 02 05 - Waste from the dairy industry
        .data[[waste_code_col]] == "02 05 01" ~ "Substances and Products Unfit for Consumption or Processing (02 05 01)",
        .data[[waste_code_col]] == "02 05 02" ~ "Sludges from On-site Wastewater Treatment (02 05 02)",
        .data[[waste_code_col]] == "02 05 99" ~ "Other Waste from Dairy Industry (02 05 99)",
        
        # 02 06 - Waste from bakeries and confectioneries
        .data[[waste_code_col]] == "02 06 01" ~ "Substances and Products Unfit for Consumption or Processing (02 06 01)",
        
        # 02 07 - Waste from the production of alcoholic and non-alcoholic beverages (except coffee, tea and cocoa)
        .data[[waste_code_col]] == "02 07 04" ~ "Materials Unsuitable for Use or Processing (02 07 04)",
        .data[[waste_code_col]] == "02 07 05" ~ "Sludges from On-site Wastewater Treatment (02 07 05)",
        ...
        .data[[waste_code_col]] == "03 03 05" ~ "Sludges from On-site Wastewater Treatment (03 03 05)",
        ...
        .data[[waste_code_col]] == "03 03 11" ~ "On-site Pulp, Paper and Cardboard Recycling Sludges (03 03 11)",
        ...
        .data[[waste_code_col]] == "07 02 99" ~ "Other Plastics, Synthetic Rubber & Fibres Waste (07 02 99)",
        
        
        # 07 03 - Wastes from the production, formulation, supply and use of organic dyes and pigments (except 06 11)
        .data[[waste_code_col]] == "07 03 03*" ~ "Organic Halogenated Solvents, Washing Liquids and Mother Liquors",
        
        # 07 05 - Waste from the manufacture, formulation, supply and use of pharmaceutical products
        .data[[waste_code_col]] == "07 05 14" ~ "Solid Wastes other than those mentioned in 07 05 13",
        ...
        .data[[waste_code_col]] == "10 01 24" ~ "Suspension Grates from Boilers (10 01 24)",
        
        # 10 09 - Waste from iron foundries
        .data[[waste_code_col]] == "10 09 08" ~ "Used Foundry Moulds and Cores, Non-Hazardous (10 09 08)",
        
        # 10 10 - Waste from non-ferrous foundries
        .data[[waste_code_col]] == "10 10 08" ~ "Waste used Foundry Moulds and Cores other than those mentioned in 10 10 07 (10 10 01)",
        # 10 12 - Waste from the manufacture of ceramic products, bricks, tiles and construction products
        .data[[waste_code_col]] == "10 12 08" ~ "Ceramic, Brick, Tile and Construction Waste (10 12 08)",
        ...
        .data[[waste_code_col]] == "15 01 07" ~ "Glass Packaging (15 01 07)",
        ...
        .data[[waste_code_col]] == "16 01 18" ~ "Non-ferrous Metals (16 01 18)",
        ...
        .data[[waste_code_col]] == "16 02 13*" ~ "Discarded Equipment Containing Hazardous Components other than those mentioned in 16 02 09 to 16 02 12 (16 02 13*)",
        ...
        .data[[waste_code_col]] == "16 03 03*" ~ "Hazardous Inorganic Wastes (16 03 03*)",
        ...
        # 16 11 - Waste linings and fire-resistant materials
        .data[[waste_code_col]] == "16 11 04" ~ "Fireproof Linings and Materials (16 11 04)",
        .data[[waste_code_col]] == "16 11 06" ~ "Non-Fireproof Linings and Materials (16 11 06)",
        
        # 17 - Construction & demolition waste
        # 17 01 - Concrete, bricks, tiles and ceramics
        .data[[waste_code_col]] == "17 01 01" ~ "Concrete Waste (17 01 01)",
        .data[[waste_code_col]] == "17 01 02" ~ "Brick Waste (17 01 02)",
        .data[[waste_code_col]] == "17 01 03" ~ "Tile and Ceramic Waste (17 01 03)",
        .data[[waste_code_col]] == "17 01 07" ~ "Non-Hazardous Concrete/Brick/Ceramic Mixtures (17 01 07)",
        
        # 17 02 - Wood, glass, and plastic
        .data[[waste_code_col]] == "17 02 01" ~ "Construction Wood Waste (17 02 01)",
        
        # 17 03 - Bituminous mixtures, coal tar and tar products
        .data[[waste_code_col]] == "17 03 02" ~ "Non-Hazardous Bituminous Mixtures (17 03 02)",
        
        # 17 04 - Metals (including alloys)
        .data[[waste_code_col]] == "17 04 05" ~ "Iron and Steel Waste (17 04 05)",
        
        # 17 05 - Soil (including excavated spoil from contaminated sites), stones and dredged material
        .data[[waste_code_col]] == "17 05 04" ~ "Gypsum-Based Waste (17 05 04)",
        .data[[waste_code_col]] == "17 05 06" ~ "Other Insulation Materials (17 05 06)",
        ...
        # 17 09 - Mixed construction and demolition wastes other than those mentioned in 17 09 01, 17 09 02 and 17 09 03
        .data[[waste_code_col]] == "17 09 04" ~ "Gypsum-Based Building Materials (17 09 04)",
        ...
        .data[[waste_code_col]] == "19 08 02" ~ "Citotoxic and Cytostatic Medicinal Wastes (19 08 02)",
        .data[[waste_code_col]] == "19 08 05" ~ "Chemicals Requiring Infectious Hazard Precautions (19 08 05)",
        .data[[waste_code_col]] == "19 08 09" ~ "Other Healthcare Wastes (19 08 09)",
        .data[[waste_code_col]] == "19 08 12" ~ "Medicinal Products (19 08 12)",
        ...
        .data[[waste_code_col]] == "19 12 08" ~ "Textiles from Mechanical Treatment (19 12 08)",
        ...
        .data[[waste_code_col]] == "20 01 25" ~ "Coal Tar Preparations for Roof Waterproofing (20 01 25)",
        ...
        .data[[waste_code_col]] == "20 01 33*" ~ "Batteries and accumulators listed under 16 06 01, 16 06 02 or 16 06 03 and unsorted batteries and accumulators containing these batteries and accumulators (20 01 33*)",
        ...
        .data[[waste_code_col]] == "20 01 37*" ~ "Wood Containing Hazardous Substances (20 01 37*)",
...
process_waste_column <- function(data, code_col, waste_desc_col) {
  ...
  # Find the newly created waste_category column
  waste_cat_cols <- names(result)[str_detect(names(result), "^waste_category(_\\d+)?$")]
  new_waste_cat_col <- waste_cat_cols[length(waste_cat_cols)] # Get the most recently added
  
  # Relocate the new waste_category column after the description column
  result |>
    relocate({{new_waste_cat_col}}, .after = {{waste_desc_col}})
}
...
json_path <- "C:/Users/kovac/Desktop/Personal/Work/Green UP Project/green-up-project/mappings.json"
...
1.  Finished up importing all the data available from the SURS website (added 2023 data)
...
## 2024 - Data from the annual reports of the original generators

### Code

```{r waste_generation_2024}
waste_generation_24.path <- "2024 data\\podatki ODP-nastajanje 2024.xlsx"

sheet_names <- excel_sheets(waste_generation_24.path)
...
for (sheet_name in sheet_names) {
  ...
  processed_sheet <- process_sheet(waste_generation_24.path, sheet_index, n_max, skip = skip)
  processed_sheets[[sheet_name]] <- processed_sheet
}

processed_sheets <- lapply(processed_sheets, filter_df_by_codes)

generation.24 <- processed_sheets[[1]]
...
english_colnames <- c(
  ...
  "type_of_waste", 
  "waste_generated",
  "temporarily_stored_start_year",
  "generated_in_the_year",
...
colnames(generation.24) <- english_colnames

generation.24 <- generation.24 |> 
  select(-report) |> 
  process_waste_data() |> 
  mutate(
    total_waste_given_away = waste_transferred_for_treatment_in_RS + waste_sent_for_treatment_EU + waste_sent_for_treatment_outside_EU
  ) |> 
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_waste_generated_stored = sum(waste_generated) / 1000,
...
write.csv(generation.24, "2024 data\\2024 filtered\\generation\\generation_24.csv", row.names = FALSE)
```

#### Plotting the composition of waste generated in the year by types

```{r new_waste_codes_2024}

# Read the data
generation.24 <- read_csv("2024 data\\2024 filtered\\generation\\generation_24.csv")

# Create a named color vector for each waste code
code_colors <- setNames(
  waste_palette[1:length(codes)],
  codes
)

# Create a named color vector for each statistical_region
region_colors <- setNames(
  waste_palette[1:length(unique(generation.24$statistical_region))],
  unique(generation.24$statistical_region)
)

# generated_in_the_year + statistical_region ----------------------------------------------------

waste_plot_data <- generation.24 |>
  ...
  mutate(statistical_region = factor(statistical_region, levels = statistical_region))
...
ggplot(waste_plot_data, 
  ...
  labs(
    title = "WASTE GENERATION BY TYPE (2024)",
...
waste_plot_data <- generation.24 |>
  ...
  arrange(total_generated) |>
  mutate(waste_code = factor(waste_code, levels = waste_code))
...
ggplot(waste_plot_data, 
  ...
  labs(
    title = "STORED WASTE GENERATION BY TYPE (2024)",
    subtitle = "Total quantities with percentage contribution",
    x = NULL,
    y = NULL
  ) +
...
## 2024 - Data on waste collected and its subsequent management from waste collectors' reports

### Code

```{r waste_collection_2024}
waste_collection_24.path <- "2024 data\\podatki ODP-zbiranje 2024.xlsx"

sheet_names <- excel_sheets(waste_collection_24.path)
...
processed_sheets <- lapply(sheet_names, function(sheet_name) {
  ...
  if (sheet_index == 1) {
    processed_sheet <- process_sheet(waste_collection_24.path, sheet_index, n_max)
  } else if (sheet_index == 6) {
    processed_sheet <- process_sheet(waste_collection_24.path, sheet_index, 1)
  } else {
    processed_sheet <- process_sheet(waste_collection_24.path, sheet_index, n_max)
  }
...
# Table 1: Waste collected, 2024
collected.24 <- processed_sheets[[1]]

collected.24 <- collected.24 |>
  select(
    year = `Leto`,
    waste_code,
...
processed_sheets[[1]] <- collected.24

# Table 1a: Waste collected by collection center, 2024
collected_center.24 <- processed_sheets[[2]] |> 
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
  ...
  process_waste_data() |> 
  group_by(year, statistical_region, collection_centre, waste_code, type_of_waste, waste_category) |>
  summarise(
    waste_collected_by_centre = sum(as.numeric(waste_collected_by_centre)) / 1000,
    .groups = "drop"
  ) 

processed_sheets[[2]] <- collected_center.24

# Table 2: Additional data on municipal waste collected and waste 16 01 03, 2024
collected_additional.24 <- processed_sheets[[3]] |> 
  select(
    year = `Leto`,
    waste_code,
    type_of_waste = `Naziv odpadka`,
  ...
  group_by(year, waste_code, type_of_waste, waste_category, waste_collected_public_service) |>
  summarise(
    total_waste_collected = sum(total_waste_collected) / 1000,
...
processed_sheets[[3]] <- collected_additional.24

# Table 3: Municipal waste collected and waste 16 01 03, separated by municipality of origin, 2024
collected_municipality.24 <- processed_sheets[[4]] |> 
  select(
    year = `Leto`,
    waste_code,
    type_of_waste = `Naziv odpadka`,
  ...
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category, name_of_municipality, waste_collected_public_service) |>
  summarise(
    waste_by_municipality = sum(waste_by_municipality) / 1000,
    .groups = "drop") 

processed_sheets[[4]] <- collected_municipality.24

# Table 4: Further management of collected waste, 2024
collected_management.24 <- processed_sheets[[5]] |> 
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    waste_code,
...
processed_sheets[[5]] <- collected_management.24

# Table 5: Household composting, 2024
# Note: composting count data not yet published for 2024
household_composting.24 <- processed_sheets[[6]] |> 
  select(
    year = `Leto`,
    name_of_municipality = `Naziv občine`,
  ...
  mutate(
    number_of_households_composting = NA_real_,
    percentage_of_households_composting = NA_real_
  )

processed_sheets[[6]] <- household_composting.24
...
for (sheet_name in sheet_names) {
  write_excel_csv(processed_sheets[[sheet_name]], paste0("2024 data\\2024 filtered\\collection\\", tolower(gsub(" ", "_", sheet_name)), "_24.csv"))
}
```

## 2024 - Data from waste treatment operators' reports

### Code

```{r waste_treatment_2024}
# Initial functions ----
waste_treatment_24.path <- "2024 data\\podatki ODP-obdelava 2024.xlsx"

sheet_names <- excel_sheets(waste_treatment_24.path)
...
# Note: sheet 1 has one extra intro row vs all other sheets; use skip=8 for it only
processed_sheets <- lapply(sheet_names, function(sheet_name) {
  sheet_index <- match(sheet_name, sheet_names)
  n_max <- if (sheet_index %in% sheets_with_n_max_3) 3 else if (sheet_index == sheets_with_n_max_4) 4 else 2
  sk <- if (sheet_index == 1) 8 else 7

  processed_sheet <- process_sheet(waste_treatment_24.path, sheet_index, n_max, skip = sk)
...
# Table 1: Storage of waste/materials or objects generated in the treatment process in previous years, as at 1.1., 2024 ----
treatment.24 <- processed_sheets[[1]] |> select(-Poročilo)

treatment.24 <- treatment.24 |>
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    treatment_inlet_waste_code,
    type_of_waste_inlet = `Naziv odpadka na vhodu v postopek obdelave`,
    treatment_outlet_waste_code,
    ...
    quantity_stored_start_year = `Skladiščena količina na dan 1.1.2024 kg`
  ) |>
...
treatment.24[] <- lapply(treatment.24, function(x) ifelse(is.na(x), "undefined", x))

treatment.24 <- treatment.24 |>
  group_by(year, statistical_region, treatment_inlet_waste_code, type_of_waste_inlet, waste_category, treatment_outlet_waste_code, type_of_waste_outlet, waste_category_1, substance_or_object) |>
  summarise(quantity_stored_start_year = sum(quantity_stored_start_year) / 1000, .groups = "drop") 

processed_sheets[[1]] <- treatment.24

# Table 2: Waste received, 2024 ----
treatment_received.24 <- processed_sheets[[2]] |> select(-Poročilo)

treatment_received.24 <- treatment_received.24 |>
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    waste_code,
  ...
  mutate(across(c(total_waste_received:waste_received_from_non_EU), as.numeric)) |>
  process_waste_column(waste_code, type_of_waste) |> 
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(
      c(
        total_waste_received,
        untreated_waste_from_storage_start_year,
        waste_received_own_waste,
        waste_received_from_generators,
        waste_received_from_collectors,
        waste_received_from_treatment,
        waste_received_from_EU,
        waste_received_from_non_EU
      ),
...
processed_sheets[[2]] <- treatment_received.24

# EXCLUDED - Table 2a: Recovered end-of-life vehicles (ELV) of categories M1, N1 and three-wheel motor vehicles (L2e), 2024 ----
treatment_ELVs.24 <- processed_sheets[[3]] |> select(-Poročilo)

# Table 3: Treatment of waste received, 2024
waste_treated.24 <- processed_sheets[[4]] |> select(-Poročilo)

waste_treated.24 <- waste_treated.24 |>
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    treatment_inlet_waste_code,
  ...
  process_waste_column(treatment_inlet_waste_code, type_of_waste) |> 
  rename_treatment_description() |> 
  group_by(year, statistical_region, treatment_inlet_waste_code, type_of_waste, waste_category,  treatment_process, description_of_treatment) |>
  summarise(
    quantity_treated = sum(quantity_treated) / 1000,
    .groups = "drop")

processed_sheets[[4]] <- waste_treated.24

# Table 4: Management of waste from the treatment process and management of untreated waste, 2024 ----
management.24 <- processed_sheets[[5]] |> 
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    treatment_inlet_delivered_waste_code,
    ...
    treatment_outlet_delivered_waste_code,
...
management.24 <- management.24 |>
  ...
  group_by(year, statistical_region, treatment_inlet_delivered_waste_code, input_waste_name, waste_category, treatment_operation, treatment_outlet_delivered_waste_code, output_waste_name, waste_category_1, further_treatment_envisaged) |>
  summarise(
...
processed_sheets[[5]] <- management.24

# Table 4a: Management of treatment products that are no longer waste, 2024 ----
management_products.24 <- processed_sheets[[6]] |> 
  select(
    year = `Leto`,
 ...
 process_waste_column(input_waste_code, type_of_waste) |> 
 filter(!is.na(type_of_waste)) |> 
 group_by(year, statistical_region, input_waste_code, type_of_waste, waste_category, treatment_operation, non_waste_substances) |>
  summarise(
  ...
  mutate(non_waste_substances = case_when(
    ...
    non_waste_substances == "22 – gradbeno polnilo" ~ "construction filler",
    TRUE ~ non_waste_substances
  ))

processed_sheets[[6]] <- management_products.24

# EXCLUDED - Table 4b: Materials resulting from the shredding of dismantled end-of-life vehicles (ELVs) of categories M1, N1 and three-wheel motor vehicles (L2e), 2024 ----
ELV_materials.24 <- processed_sheets[[7]] |> select(-Poročilo)

# EXCLUDED - Table 5: Mechanical-biological treatment of mixed municipal waste, separated by municipality of origin, 2024 ----
MBT_municipality.24 <- processed_sheets[[8]] |> select(-Poročilo)

# Table 6: Mass difference between waste input and treatment product volumes, 2024 ----
mass_difference.24 <- processed_sheets[[9]] |> select(-Poročilo)

mass_difference.24 <- mass_difference.24 |>
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    input_waste_code,
  ...
  process_waste_column(input_waste_code, input_waste_name) 

mass_difference.24 <- mass_difference.24 |>
  group_by(year, statistical_region, input_waste_code, input_waste_name, waste_category, treatment_operation) |>
  summarise(mass_difference = sum(mass_difference) / 1000, .groups = "drop") 

processed_sheets[[9]] <- mass_difference.24

# EXCLUDED - Table 7: Landfill capacity and status, 2024 ----
landfill_capacity.24 <- processed_sheets[[10]] |> select(-Poročilo)

# Table 8: Waste management in landfill, 2024 ----
landfill_management.24 <- processed_sheets[[11]] |> 
  select(
    year = `Leto`,
  ...
  process_waste_column(waste_code, type_of_waste) |>
  group_by(year, landfill, waste_code, type_of_waste, waste_category) |>
  summarise(
...
processed_sheets[[11]] <- landfill_management.24

# EXCLUDED - Table 9: Disposed waste resulting from municipal waste treatment (from mechanical-biological treatment of mixed municipal waste and from treatment operations of separated fractions of municipal waste), by municipality of origin, 2024 ----
disposed_municipality.24 <- processed_sheets[[12]] |> select(-Poročilo)

# EXCLUDED - Table 10: Compost or digestate production, 2024 ----
compost_digestate.24 <- processed_sheets[[13]] |> select(-Poročilo)

# EXCLUDED - Table 11: Use of compost or digestate as waste, 2024 ----
compost_digestate_use.24 <- processed_sheets[[14]] |> select(-Poročilo)

# EXCLUDED - Table 12: Additional data on waste oils, 2024 ----
waste_oils.24 <- processed_sheets[[15]] |> select(-Poročilo)
...
# Write files ----
for (sheet_name in sheet_names) {
  write_excel_csv(processed_sheets[[sheet_name]], paste0("2024 data\\2024 filtered\\treatment\\", tolower(gsub(" ", "_", sheet_name)), "_24.csv"))
}
```
...
english_colnames <- c(
  ...
  "waste_code", 
  "type_of_waste", 
  "waste_generated", # total_waste_generated_stored waste_stored_start_year
  "temporarily_stored_start_year", # waste_stored_end_year
  "generated_in_the_year", # waste_generated
  "temporarily_stored_end_year",
  "waste_treated_by_original_generator",
...
generation.23 <- generation.23 |> 
  ...
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    # total_waste_generated_stored = temporarily_stored_start_year + generated_in_the_year
    total_waste_generated_stored = sum(waste_generated) / 1000, # convert to tons
    temporarily_stored_start_year = sum(temporarily_stored_start_year) / 1000,
    generated_in_the_year = sum(generated_in_the_year) / 1000,
    temporarily_stored_end_year = sum(temporarily_stored_end_year) / 1000,
    # total_waste_given_away = total_waste_generated_stored - waste_treated_by_original_generator
    total_waste_given_away = sum(total_waste_given_away) / 1000,
    waste_treated_by_original_generator = sum(waste_treated_by_original_generator) / 1000,
    waste_transferred_for_treatment_in_RS = sum(waste_transferred_for_treatment_in_RS) / 1000,
    waste_sent_for_treatment_EU = sum(waste_sent_for_treatment_EU) / 1000,
...
collected.23 <- processed_sheets[[1]]
...
collected_center.23 <- processed_sheets[[2]] |> 
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    collection_centre = `Zbirni center`,
...
collected_additional.23 <- processed_sheets[[3]] |> 
  select(
    year = `Leto`,
    waste_code,
    type_of_waste = `Naziv odpadka`,
...
collected_municipality.23 <- processed_sheets[[4]] |> 
  select(
    year = `Leto`,
    waste_code,
    type_of_waste = `Naziv odpadka`,
...
collected_management.23 <- processed_sheets[[5]] |> 
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    waste_code,
...
household_composting.23 <- processed_sheets[[6]] |> 
  select(
    year = `Leto`,
    name_of_municipality = `Naziv občine`,
    number_of_households_in_municipality = `Število gospodinjstev v občini 2`,
...
# add
# [1] "16 11 04" "17 01 02" "17 01 07" "17 03 02" "17 05 04" "17 05 06"
# [7] "17 09 04" "10 01 24" "17 01 03" "07 02 99" "01 04 13" "02 01 03"
# [13] "02 03 04" "10 09 08" "10 12 08" "19 08 02" "20 01 25" "02 01 06"
# [19] "02 02 01" "02 02 04" "02 03 01" "02 03 05" "02 05 01" "02 05 02"
# [25] "02 05 99" "02 06 01" "02 07 04" "02 07 05" "03 03 11" "16 11 06"
# [31] "19 08 09" "19 08 05" "19 08 12" "04 01 09" "12 01 01"
...
management.23 <- processed_sheets[[5]] |> 
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    treatment_inlet_delivered_waste_code,
...
management_products.23 <- processed_sheets[[6]] |> 
  select(
    year = `Leto`,
    statistical_region = `Poročilo Statistična regija`,
    input_waste_code,
...
landfill_management.23 <- processed_sheets[[11]] |> 
  select(
    year = `Leto`,
    landfill = `Odlagališče`,
    waste_code,
...
pre_collection.22 <- processed_sheets[[1]] |> 
  select(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    waste_code,
...
collected_mun.22 <- processed_sheets[[3]] |> 
  ...
  process_waste_column(waste_code, type_of_waste) |> 
  group_by(year, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_mun_waste_collected = sum(total_mun_waste_collected) / 1000,
    mun_waste_collected_by_househoulds_gen = sum(mun_waste_collected_by_househoulds_gen) / 1000,
    mun_waste_collected_by_companies_gen = sum(mun_waste_collected_by_companies_gen) / 1000,
    .groups = "drop"
  )
...
collected_org_mun.22 <- processed_sheets[[4]] |> 
  ...
  left_join(municipality_region_mapping, by = "name_of_municipality") |> 
  group_by(year, statistical_region, name_of_municipality, waste_code, type_of_waste, waste_category) |>
  summarise(
    waste_by_municipality = sum(waste_by_municipality) / 1000, 
    .groups = "drop"
  )
...
composting.22 <- processed_sheets[[6]] |> 
  ...
  left_join(municipality_region_mapping, by = "name_of_municipality") |> 
  mutate(
    percentage_of_households_composting = round(number_of_households_composting / number_of_households * 100, 2)
  )

processed_sheets[[6]] <- composting.22 |> select(
  year, 
  statistical_region, 
  name_of_municipality,
  number_of_households,
  number_of_households_composting,
  percentage_of_households_composting
)
...
treatment.22 <- processed_sheets[[1]] |> 
  ...
  # if waste_code is NA, drop the rows containg NA values
...
waste_treated.22 <- processed_sheets[[3]] |>
  ...
  process_waste_column(waste_code, type_of_waste) |> 
  rename_treatment_description() |>
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category, treatment_process, description_of_treatment) |>
  summarize(
    waste_quantity_per_treatment = sum(waste_quantity_per_treatment) / 1000,
    .groups = "drop"
  ) 
...
mech_bio_treatment.22 <- processed_sheets[[6]] |> 
  ...
  summarise(
    mixed_waste_mech_bio_treatment = sum(mixed_waste_mech_bio_treatment) / 1000,
  .groups = "drop"
  )
...
mass_difference.22 <- processed_sheets[[7]] |> dplyr::select(-Poročilo, -`Odpadne sveče pri odpadku 20 02 03 (da/ne)`)
...
mass_difference.22 <- mass_difference.22 |>
  ...
  mutate(
    reduction_or_increase_of_mass_during_treatment = as.numeric(reduction_or_increase_of_mass_during_treatment) / 1000) 
...
# Lesena embalaža
generation.21 <- process_waste_column(generation.21, waste_code, type_of_waste)
...
pre_collection.21 <- processed_sheets[[1]] |>
...
pre_collection.21 <- pre_collection.21 |>
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    previous_stored_start_year = sum(previous_stored_start_year) / 1000,
    previous_stored_end_year = sum(previous_stored_end_year) / 1000,
...
processed_sheets[[1]] <- pre_collection.21

# Sheet 2: Waste collected, 2021 ----
## Rename the columns of the second sheet
collected.21 <- processed_sheets[[2]] |>
  select(-Poročilo, -`Odpadne nagrobne sveče (da/ne)`) |>
  rename(
    year = `Leto`,
    statistical_region = `Statistična regija`,
...
processed_sheets[[2]] <- collected.21
...
collected_mun.22 <- process_waste_column(collected_mun.22, waste_code, type_of_waste)
...
wc_by_origin <- processed_sheets[[4]] |>
  ...
  process_waste_column(waste_code, type_of_waste) |> 
  group_by(year, name_of_municipality, waste_code, type_of_waste, waste_category) |>
  summarise(waste_by_municipality = sum(waste_by_municipality) / 1000, .groups = "drop")
...
processed_sheets[[4]] <- wc_by_origin |> select(
  year,
  statistical_region,
  name_of_municipality,
  waste_code, 
  type_of_waste,
  waste_category,
  waste_by_municipality
)

# Sheet 5: Management of collected waste, 2021 ----
## Rename the columns of the fifth sheet
management.21 <- processed_sheets[[5]] |>
  select(-Poročilo, -`Odpadne nagrobne sveče (da/ne)`) |>
  rename(
    statistical_region = `Statistična regija`,
    year = `Leto`,
...
processed_sheets[[5]] <- management.21

# Sheet 6: Household composting, 2021 ----
## Rename the columns of the sixth sheet
composting.21 <- processed_sheets[[6]] |>
...
processed_sheets[[6]] <- composting.21 |> select(
  year,
  statistical_region,
  name_of_municipality,
  number_of_households,
  ...
  summarise(
    across(
      c(
        total_waste_received,
        waste_received_own_waste,
        waste_received_from_producers_RS,
        waste_received_from_collectors_RS,
        waste_received_from_processors_RS,
        waste_received_from_EU,
        waste_received_from_non_EU
      ),
      ~ sum(.x, na.rm = TRUE) / 1000
...
treating_waste.21 <- treating_waste.21 |>
  ...
  process_waste_column(waste_code, type_of_waste) |>
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category, treatment_process) |>
  summarise(waste_entering_treatment_process = sum(waste_entering_treatment_process) / 1000, .groups = "drop") 
...
management_waste.21 <- management_waste.21 |>
  group_by(year, statistical_region, treatment_inlet_delivered_waste_code, input_waste_name, waste_category, treatment_outlet_delivered_waste_code, output_waste_name, waste_category_1) |>
  summarise(
    waste_handed_to_collectors_RS = sum(waste_handed_to_collectors_RS) / 1000,
    waste_delivered_to_operators_RS = sum(waste_delivered_to_operators_RS) / 1000,
    waste_sent_to_EU = sum(waste_sent_to_EU) / 1000,
...
management_waste.21a <- management_waste.21a |> process_waste_column(input_waste_code, input_waste_name)
...
management_waste.21a <- management_waste.21a |>
  group_by(year, input_waste_code, input_waste_name, waste_category, output_waste_name) |>
  summarise(
...
waste_treatment.21 <- waste_treatment.21 |>
  dplyr::select(-Poročilo, -`Odpadne nagrobne sveče (da/ne)`, -`Šifra občine 1`) |>
  rename(
    year = `Leto`,
...
waste_treatment.21 <- process_waste_column(waste_treatment.21, waste_code, type_of_waste)
...
mass_difference.21 <- mass_difference.21 |>
  dplyr::select(-Poročilo, -`Odpadne nagrobne sveče (da/ne)`) |>
  rename(
    year = `Leto`,
    input_waste_name = `Odpadek na vhodu v postopek obdelave naziv`,
...
mass_difference.21 <- mass_difference.21 |>
  group_by(year, input_waste_code, input_waste_name, waste_category) |>
...
landfill_waste.21 <- process_waste_column(landfill_waste.21, waste_code, type_of_waste)
...
# EXCLUDED - Sheet 9: Municipal waste disposed by municipality of origin ----
...
generation.20 <- processed_sheets[[1]] |>
  select(
    ...
    waste_sent_for_treatment_EU = `Odpadki poslani v obdelavo v druge države članice EU kg`,
    waste_sent_for_treatment_outside_EU = `Odpadki poslani v obdelavo v države izven EU kg`
  ) |> 
  process_waste_column(waste_code, type_of_waste) |> 
  mutate(
    total_waste_given_away = waste_handed_to_others_RS + waste_sent_for_treatment_EU + waste_sent_for_treatment_outside_EU
  ) |>
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
...
# to csv
write_excel_csv(generation.20, "2020 data\\2020 filtered\\generation\\generation_20.csv")
```
...
waste_storage.20 <- processed_sheets[[1]] |>
  # 1) select & rename only what you need
  select(
    year                   = Leto,
    statistical_region     = `Statistična regija`,
    waste_code,
  ...
  # 2) categorize
  process_waste_column(waste_code, type_of_waste) |>
  # 3) convert both kg‐columns to tonnes
  mutate(
    across(
      c(waste_stored_start_year, waste_stored_end_year),
      ~ .x / 1000
    )
  )
...
collected_waste.20 <- processed_sheets[[2]] |>
  ...
  mutate(
    across(
      c(
        waste_received_from_producers_no_list,
        waste_received_from_producers_with_list,
        waste_received_from_collectors_RS,
        waste_received_from_processors_RS
      ),
      ~ .x / 1000
    ),
    total_waste_collected = rowSums(
      across(
        c(
          waste_received_from_producers_no_list,
          waste_received_from_producers_with_list,
...
municipal_waste.20 <- processed_sheets[[3]] |>
  # 1) pick & rename exactly the columns you need
  select(
    year                                   = Leto,
    waste_code,
    type_of_waste                          = `Naziv odpadka`,
  ...
  # 2) process code + type into categories
  process_waste_column(waste_code, type_of_waste) |>
  # 3) convert all kg-columns to tonnes in one go
  mutate(
    across(
      c(total_municipal_waste_collected,
        municipal_waste_collected_from_households,
        municipal_waste_collected_from_activities),
      ~ .x / 1000
    )
  )
...
municipal_waste_origin.20 <- processed_sheets[[4]] |>
  ...
  select(
    ...
    type_of_waste,
    waste_category,
    municipal_waste_collected_by_municipality
  )
...
management_collected_waste.20 <- processed_sheets[[5]] |>
  select(
    ...
    waste_sent_to_EU                = `Odpadki oddani v nadaljnje ravnanje v druge države članice EU kg`,
    waste_sent_to_non_EU            = `Odpadki oddani v nadaljnje ravnanje v države izven EU kg`
    # waste_code stays the same
  ) |>
  process_waste_column(waste_code, type_of_waste) |>
  mutate(
    across(
      c(waste_handed_to_collectors_RS,
        waste_delivered_to_operators_RS,
        waste_sent_to_EU,
        waste_sent_to_non_EU),
      ~ .x / 1000
    ),
    total_waste_given_away = rowSums(
      across(
        c(waste_handed_to_collectors_RS,
          waste_delivered_to_operators_RS,
          waste_sent_to_EU,
  ...
  select(
    year, statistical_region, waste_code,
    type_of_waste, waste_category,
...
home_composting.20 <- processed_sheets[[6]] |>
  select(
    ...
    number_of_households           = `Število gospodinjstev na občino 2 7035`,
    number_of_households_composting = `Število gospodinjstev na občino, ki hišno kompostirajo 1740`
  ) |>
  left_join(municipality_region_mapping, by = "name_of_municipality") |>
  mutate(
    percentage_of_households_composting =
      round(number_of_households_composting / number_of_households * 100, 2)
  ) |>
  select(
    year,
    statistical_region,
...
# Define the sheets where n_max should be 3 (replace with actual sheet indices)
sheets_with_n_max_3 <- c(2, 4, 5, 7, 10)
...
treating_waste.20 <- processed_sheets[[3]] |>
  ...
  summarise(
    across(
      waste_entering_treatment_process,
      ~ sum(.x, na.rm = TRUE) / 1000
    ),
    .groups = "drop"
  )
...
management_waste.20a <- processed_sheets[[5]] |>
  ...
  process_waste_column(input_waste_code, input_waste_name) |>
  mutate(
    output_waste_name = case_when(
      output_waste_name == "les"                 ~ "wood",
      output_waste_name == "drugo"               ~ "other",
      output_waste_name == "gradbeni proizvodi"  ~ "construction product",
      output_waste_name == "tekstil in usnje"    ~ "textile and leather",
      TRUE                                        ~ output_waste_name
    )
  ) |>
  group_by(year, input_waste_code, input_waste_name, waste_category, output_waste_name) |>
  summarise(
    across(
      c(delivered_to_storage_or_other_persons_RS, shipped_to_EU, shipped_to_non_EU),
      ~ sum(.x, na.rm = TRUE) / 1000
    ),
    .groups = "drop"
  )
...
waste_treatment.20 <- processed_sheets[[6]] |>
  ...
  group_by(year, statistical_region, name_of_municipality, waste_code, type_of_waste, waste_category) |>
  summarise(
    waste_collected_by_municipality = sum(waste_collected_by_municipality, na.rm = TRUE) / 1000,
    .groups = "drop"
  )
...
mass_difference.20 <- processed_sheets[[7]] |>
  ...
  group_by(year, input_waste_code, input_waste_name, waste_category) |>
  summarize(
    mass_change = sum(as.numeric(mass_change), na.rm = TRUE) / 1000,
    .groups = "drop"
  )
...
landfill_capacity.20 <- processed_sheets[[8]] |>
  # 1) drop the report column & rename
  dplyr::select(
    year                        = Leto,
    landfill_site               = Odlagališče,
    volume_filled_start_year    = `Zapolnjen volumen na odlagalnem polju na začetku leta m3`,
  ...
  # 2) recode landfill names in one mutate()
  mutate(
    landfill_site = case_when(
      landfill_site == "Odlagališče 1"  ~ "Landfill 1",
      landfill_site == "Odlagališče 2"  ~ "Landfill 2",
      landfill_site == "Odlagališče 3"  ~ "Landfill 3",
...
landfill_waste.20 <- processed_sheets[[9]] |>
  # 1) drop the report column and rename everything in one step
  dplyr::select(
    year                           = Leto,
    waste_code,
    type_of_waste                  = `Naziv odpadka`,
  ...
  # 2) categorize
  process_waste_column(waste_code, type_of_waste) |>
  # 3) drop any all-NA columns
  select(where(~ !all(is.na(.)))) |>
  # 4) aggregate ALL eight metrics (sum & convert to tonnes)
  group_by(year, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(
      c(
        total_waste_received,
        waste_without_assessment,
        disposed_waste_total,
        disposed_waste_hazardous,
        disposed_waste_non_hazardous,
        disposed_waste_inert,
        disposed_waste_for_construction,
        disposed_waste_for_covering
      ),
      ~ sum(as.numeric(.x), na.rm = TRUE) / 1000
    ),
    .groups = "drop"
  )
...
# EXCLUDED - Table 9: Municipal waste disposed of by municipality of origin in 2020 ----
...
for (sheet_name in sheet_names) {
  sheet_index <- match(sheet_name, sheet_names)
  processed_sheet <- process_sheet(waste_generation_19.path, sheet_index)
  processed_sheets[[sheet_name]] <- processed_sheet
}
...
generation.19 <- processed_sheets[[1]] |>
  ...
  process_waste_column(waste_code, type_of_waste) |> 
  mutate(
    total_waste_given_away = waste_handed_to_others_RS + 
      waste_sent_for_treatment_EU + 
      waste_sent_for_treatment_outside_EU
  ) |> 
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category) |>
...
# to csv
write_excel_csv(generation.19, "2019 data\\2019 filtered\\generation\\generation_19.csv")
```
...
waste_collection_19.sheet1 <- read_xlsx(waste_collection_19.path, skip = 2, sheet = 1)
...
waste_collection_19.by_codes1 <- waste_collection_19.sheet1 |>
  ...
  filter_codes() |> 
  categorize_waste() |> 
  group_by(year = 2019, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarize(
...
waste_collection_19.sheet1 <- waste_collection_19.by_codes1

# Table 2: Waste collected in 2019
waste_collection_19.sheet2 <- read_xlsx(waste_collection_19.path, skip = 2, sheet = 2)
...
waste_collection_19.by_codes2 <- waste_collection_19.sheet2 |>
  ...
  filter_codes() |>
  categorize_waste() |>
  # 3) to tonnes + recode the source labels
  mutate(
    year           = 2019,
    waste_received = waste_received / 1000,
    gathered_from = recode(
      gathered_from,
      "1-izvirni povzrocitelj: prepušceno (brez evidencnega lista)" = "from_producers_no_list",
      "2-izvirni povzrocitelj: oddano (z evidencnim listom)"        = "from_producers_with_list",
      "3-drug zbiralec"                                             = "from_other_collectors",
      "4-izvajalec obdelave"                                       = "from_processors"
    )
  ) |>
  # 4) sum per group × source
  group_by(
    year, name_of_company, statistical_region,
    waste_code, type_of_waste, waste_category, gathered_from
  ) |>
  summarise(
    tonnage = sum(waste_received, na.rm = TRUE),
    .groups = "drop"
  ) |>
  # 5) pivot those four source‐rows into columns
  pivot_wider(
    names_from   = gathered_from,
    values_from  = tonnage,
    names_prefix = "waste_",
    values_fill  = 0
  ) |>
  # 6) recompute the overall received total (optional)
  mutate(
    waste_received = waste_from_producers_no_list +
                     waste_from_producers_with_list +
                     waste_from_other_collectors +
                     waste_from_processors
  ) |> 
  select(
    year, name_of_company, statistical_region,
    waste_code, type_of_waste, waste_category,
    waste_received,
    waste_from_producers_no_list,
    waste_from_producers_with_list,
    waste_from_other_collectors,
    waste_from_processors
  )

waste_collection_19.sheet2 <- waste_collection_19.by_codes2

# Table 3: Waste collected in 2019 by municipality of origin
waste_collection_19.sheet3 <- read_xlsx(waste_collection_19.path, skip = 2, sheet = 3)

waste_collected_municipality.19 <- waste_collection_19.sheet3 |>
  select(
    waste_code = `ŠT. ODPADKA`,
  ...
  mutate(
    year = 2019,
    across(
      c(total_waste_collected, 
        municipal_waste_collected, 
        waste_collected_from_activities),
      ~ coalesce(.x, 0) / 1000
    )
  ) |> 
  group_by(
  ...
  summarise(
    total_waste_collected         = sum(total_waste_collected, na.rm = TRUE),
    municipal_waste_collected     = sum(municipal_waste_collected, na.rm = TRUE),
    waste_collected_from_activities = sum(waste_collected_from_activities, na.rm = TRUE),
    .groups = "drop"
  ) |> 
  mutate(
    pct_municipal = 
      if_else(total_waste_collected > 0, 
              municipal_waste_collected / total_waste_collected * 100, 
              NA_real_)
  )
...
municipal_waste_origin_19.sheet4 <- read_xlsx(waste_collection_19.path, skip = 2, sheet = 4)
...
municipal_waste_origin_19.by_codes4 <- municipal_waste_origin_19.sheet4 |>
  ...
  categorize_waste() |> 
  left_join(municipality_region_mapping, by = "name_of_municipality") |> 
  group_by(year = 2019, statistical_region, name_of_municipality, waste_code, type_of_waste, waste_category) |>
  summarise(total_waste_collected = 
              sum(total_waste_collected) / 1000,
            .groups = "drop"
            ) 

municipal_waste_origin_19.sheet4 <- municipal_waste_origin_19.by_codes4

# Table 5: Handling of Collected Waste in 2019
waste_handling_19.sheet5 <- read_xlsx(waste_collection_19.path, skip = 2, sheet = 5)
...
waste_collection.19 <- list(
  waste_collection_19.sheet1,
  waste_collection_19.sheet2,
  waste_collection_19.sheet3,
  municipal_waste_origin_19.sheet4,
  waste_handling_19.by_codes5
)
...
waste_recovery_19.sheet1 <- read_xlsx(waste_recovery_19.path, skip = 2, sheet = 1)

waste_storage.19 <- waste_recovery_19.sheet1 |>
  dplyr::select(
    name_of_company = `NAZIV`,
    statistical_region = `STATISTIČNA REGIJA`,
    waste_code = `ŠT. ODPADKA`,
...
waste_recovery_19.sheet1 <- waste_storage.19
```

```{r waste_collected}
# WASTE COLLECTED IN 2019

waste_recovery_19.sheet2 <- read_xlsx(waste_recovery_19.path, skip = 2, sheet = 2)

waste_collected.19 <- waste_recovery_19.sheet2 |>
  dplyr::select(
    name_of_company = `NAZIV`,
    statistical_region = `STATISTIČNA REGIJA`,
    waste_code = `ŠT. ODPADKA`,
  ...
  categorize_waste() |> 
  # solve NA for numeric values
  mutate(total_waste_received = replace_na(total_waste_received, 0)) |>
  mutate(
    year = 2019,  
  ...
  mutate(
    waste_received_from_producer = ifelse(received_from == "own waste", total_waste_received, 0),
    waste_received_from_other_producer = ifelse(received_from == "other producer in RS", total_waste_received, 0),
    waste_received_from_collector = ifelse(received_from == "other collector in RS", total_waste_received, 0),
    waste_received_from_processing_operator = ifelse(received_from == "other processing operator in RS", total_waste_received, 0),
    waste_received_from_other_EU_country = ifelse(received_from == "other EU country", total_waste_received, 0),
    waste_received_from_other_country_outside_EU = ifelse(received_from == "other country outside EU", total_waste_received, 0)
  ) |> 
  group_by(year, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(c(total_waste_received, starts_with("waste_received_from_")), 
           ~ sum(.x, na.rm = TRUE) / 1000),
    .groups = "drop"
  )

waste_recovery_19.sheet2 <- waste_collected.19
```

```{r treatment_waste_received}
# TREATMENT OF WASTE RECEIVED IN 2019

waste_recovery_19.sheet3 <- read_xlsx(waste_recovery_19.path, skip = 2, sheet = 3)

waste_treatment.19 <- waste_recovery_19.sheet3 |>
  dplyr::select(
    name_of_company = `NAZIV`,
    statistical_region = `STATISTIČNA REGIJA`,
    waste_code = `ŠT. ODPADKA`,
  ...
  mutate(across(everything(), ~ replace_na(.x, 0))) |>
  mutate(
    treatment_according_to_8th_article = case_when(
      treatment_according_to_8th_article == "DA" ~ "YES",
      treatment_according_to_8th_article == "NE" ~ "NO"
    )
  ) |> 
  group_by(year = 2019, name_of_company, statistical_region, type_of_waste, waste_category, treatment_process, treatment_according_to_8th_article) |>
  summarise(
...
waste_recovery_19.sheet4 <- read_xlsx(waste_recovery_19.path, skip = 2, sheet = 4)

waste_treatment_process.19 <- waste_recovery_19.sheet4 |> 
  dplyr::select(
    name_of_company = `NAZIV`,
    statistical_region = `STATISTIČNA REGIJA`,
    waste_code = `ŠT. ODPADKA`,
...
waste_recovery_19.sheet4 <- waste_treatment_process.19
```
...
waste_recovery_19.sheet5 <- read_xlsx(waste_recovery_19.path, skip = 2, sheet = 5)

municipal_waste_treatment.19 <- waste_recovery_19.sheet5 |>
  dplyr::select(
    waste_code = `ŠT. ODPADKA`, 
    name_of_company = `NAZIV`,
...
municipal_waste_treatment.19 <- municipal_waste_treatment.19 |>
  group_by(year = 2019, name_of_company, statistical_region, name_of_municipality, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_waste_received = sum(total_waste_received) / 1000,
    .groups = "drop"
    ) 
...
waste_recovery_19.sheet6 <- read_xlsx(waste_recovery_19.path, skip = 2, sheet = 6)

waste_difference.19 <- waste_recovery_19.sheet6 |>
  dplyr::select(-`MATIČNA ŠTEVILKA`, -SKD) |>
  rename(
    name_of_company = `NAZIV`,
    waste_code = `ŠT. ODPADKA`,
...
waste_recovery_19.sheet6 <- waste_difference.19 |>
  dplyr::select(year, name_of_company, waste_code, type_of_waste, waste_category, lost_mass)
```

```{r landfill_capacity_EXCLUDED}
# LANDFILL CAPACITY AND STATUS IN 2019 - EXCLUDED
waste_recovery_19.sheet7 <- read_xlsx(waste_recovery_19.path, skip = 2, sheet = 7)
...
waste_recovery_19.sheet8 <- read_xlsx(waste_recovery_19.path, skip = 2, sheet = 8)

waste_volumes_landfill.19 <- waste_recovery_19.sheet8 |>
  dplyr::select(
    name_of_company = `NAZIV`,
    landfill = `ODLAGALIŠČE`,
    waste_code = `ŠT. ODPADKA`,
...
## Drop columns containing NA values
  ...
  # solve NA for numeric values
  mutate(across(everything(), ~ replace_na(.x, 0))) |>
  group_by(year = 2019, landfill, waste_code, type_of_waste, waste_category) |>
  summarise(
...
waste_recovery_19.sheet8 <- waste_volumes_landfill.19 |> dplyr::select(year, landfill, waste_code, type_of_waste, waste_category, non_hazardous_waste_received, disposed)
```

```{r municipal_waste_disposed_EXCLUDED}
# MUNICIPAL WASTE DISPOSED OF BY MUNICIPALITY IN 2019 - EXCLUDED
waste_recovery_19.sheet9 <- read_xlsx(waste_recovery_19.path, skip = 2, sheet = 9)
...
data18.sheet1 <- read_xlsx(waste_generation_18.path, skip = 2, sheet = 1)
...
data18.sheet2 <- read_xlsx(waste_generation_18.path, skip = 2, sheet = 2)

data18.sheet2 <- data18.sheet2 |>
  select(
    name_of_company       = NAZIV,
    statistical_region    = colnames(data18.sheet2)[3],
    waste_code            = `ŠT. ODPADKA`,
  ...
  mutate(
    ...
    delivered_to = recode(
      delivered_to,
      "1" = "self_processed",
      "2" = "processing_operator_RS",
      "3" = "other_EU_country",
      "4" = "other_country_outside_EU"
    )
  ) |>
  rename_region() |>
  mutate(
    waste_treated_by_producer            = if_else(delivered_to == "self_processed",             total_waste_given_away, 0),
    waste_handed_to_others_RS            = if_else(delivered_to == "processing_operator_RS",    total_waste_given_away, 0),
    waste_sent_to_EU       = if_else(delivered_to == "other_EU_country",          total_waste_given_away, 0),
    waste_sent_for_treatment_outside_EU = if_else(delivered_to == "other_country_outside_EU", total_waste_given_away, 0)
  ) |>
  group_by(
    year, name_of_company, statistical_region,
    waste_code, type_of_waste, waste_category
...
```{r waste_storage}
waste_collection_18.sheet1 <- read_xlsx(waste_collection_18.path, skip = 2, sheet = 1)
...
```{r waste_collected}
waste_collection_18.sheet2 <- read_xlsx(waste_collection_18.path, skip = 2, sheet = 2)
...
waste_collection_18.by_codes2 <- waste_collection_18.sheet2 |>
  ...
  mutate(
    ...
    waste_from_producers_no_record = ifelse(collected_from == "received from producers without a list", total_waste_collected, 0),
    waste_from_producers_with_record = ifelse(collected_from == "received from producers with a list", total_waste_collected, 0),
    waste_from_collectors_RS = ifelse(collected_from == "received from other collectors", total_waste_collected, 0),
    waste_from_processors_RS = ifelse(collected_from == "received from processing operators", total_waste_collected, 0)
  ) |>
  group_by(year = 2018, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(c(total_waste_collected, waste_from_producers_no_record, 
             waste_from_producers_with_record, waste_from_collectors_RS, 
             waste_from_processors_RS), ~ sum(.x) / 1000),
    .groups = "drop"
  )
```

```{r municipal_waste_collected}
waste_collection_18.sheet3 <- read_xlsx(waste_collection_18.path, skip = 2, sheet = 3)
...
waste_collection_18.by_codes3 <- waste_collection_18.sheet3 |>
  ...
  group_by(year, name_of_company, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_waste_collected = sum(total_waste_collected) / 1000,
...
waste_collection_18.sheet4 <- read_xlsx(waste_collection_18.path, skip = 2, sheet = 4)
...
waste_collection_18.by_codes4 <- waste_collection_18.sheet4 |>
  ...
  group_by(year, name_of_company, statistical_region, name_of_municipality, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_waste_collected = sum(total_waste_collected) / 1000,
    .groups = "drop"
  ) 
...
waste_collection_18.sheet5 <- read_xlsx(waste_collection_18.path, skip = 2, sheet = 5)
...
waste_collection_18.by_codes5 <- waste_collection_18.sheet5 |>
  ...
  group_by(year = 2018, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_waste_given_away = sum(total_waste_given_away) / 1000,
...
waste_recovery_18.sheet1 <- read_xlsx(waste_recovery_18.path, skip = 2, sheet = 1)
...
waste_recovery_18.sheet2 <- read_xlsx(waste_recovery_18.path, skip = 2, sheet = 2)
...
waste_recovery_18.by_codes2 <- waste_recovery_18.sheet2 |>
  ...
  mutate(
    received_from_own_waste_OVD = ifelse(received_from == "own waste, 'OVD' case", total_waste_received, 0),
    received_from_other_producer_RS = ifelse(received_from == "other original producer in RS", total_waste_received, 0),
    received_from_other_collector_RS = ifelse(received_from == "other collector in RS", total_waste_received, 0),
    received_from_other_processing_operator_RS = ifelse(received_from == "other processing operator in RS", total_waste_received, 0),
    received_from_other_EU_country = ifelse(received_from == "other EU country", total_waste_received, 0),
    received_from_other_country_outside_EU = ifelse(received_from == "other country (outside EU)", total_waste_received, 0)
  ) |> 
  group_by(year, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(c(total_waste_received, starts_with("received_from_")), 
           ~ sum(.x, na.rm = TRUE) / 1000),
    .groups = "drop"
  )
```
...
waste_recovery_18.sheet3 <- read_xlsx(waste_recovery_18.path, skip = 2, sheet = 3)
...
waste_recovery_18.by_codes3 <- waste_recovery_18.sheet3 |>
  ...
  group_by(year = 2018, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, treatment_process, treatment_according_to_8th_article) |>
  summarise(
    total_waste_given_away = sum(total_waste_given_away) / 1000,
    .groups = "drop"
  ) 
...
waste_recovery_18.sheet4 <- read_xlsx(waste_recovery_18.path, skip = 2, sheet = 4)
...
waste_recovery_18.sheet5 <- read_xlsx(waste_recovery_18.path, skip = 2, sheet = 5)
...
waste_recovery_18.by_codes5 <- waste_recovery_18.sheet5 |>
  ...
  process_waste_column(waste_code, type_of_waste) |>
  left_join(municipality_region_mapping, by = "name_of_municipality") |> 
  group_by(year = 2018, name_of_company, statistical_region, name_of_municipality, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_waste_received = sum(total_waste_received) / 1000,
    .groups = "drop"
  ) 
...
waste_recovery_18.sheet6 <- read_xlsx(waste_recovery_18.path, skip = 2, sheet = 6)
...
waste_recovery_18.by_codes6 <- waste_recovery_18.sheet6 |>
  ...
  filter_codes() |> 
  process_waste_column(waste_code, type_of_waste) |>
  group_by(year = 2018, name_of_company, waste_code, type_of_waste, waste_category) |>
  summarise(
...
waste_recovery_18.sheet7 <- read_xlsx(waste_recovery_18.path, skip = 2, sheet = 7)
...
waste_recovery_18.sheet8 <- read_xlsx(waste_recovery_18.path, skip = 2, sheet = 8)
...
waste_recovery_18.by_codes8 <- waste_recovery_18.sheet8 |>
  ...
  process_waste_column(waste_code, type_of_waste) |>
  select_if(~ !all(is.na(.))) |> 
  group_by(year = 2018, name_of_company, landfill, waste_code, type_of_waste, waste_category) |>
  summarise(
...
waste_recovery_18.sheet9 <- read_xlsx(waste_recovery_18.path, skip = 2, sheet = 9)
...
waste_generation_17.sheet1 <- read_xlsx(waste_generation_17.path, skip = 2, sheet = 1)
...
waste_generation_17.by_codes1 <- waste_generation_17.sheet1 |>
  rename(
    ...
    total_waste_generated_kg  = `KOLIČINA ODPADKOV, NASTALIH V LETU 2017 (kg)`,
    waste_stored_end_year     = `SKLADIŠČENA KOLIČINA  ODPADKOV NA DAN 31.12.2017 (kg)`
  ) |> 
  filter(
    waste_code %in% codes |
    (str_detect(waste_code, "^03 01 [0-9]") & !str_detect(waste_code, "03 01 04"))
  ) |> 
  categorize_waste()  |> 
  mutate(
    across(
      c(total_waste_2017_kg, waste_stored_start_year, total_waste_generated_kg, waste_stored_end_year),
      ~ coalesce(.x, 0) / 1000
    ),
    total_waste_2017 = total_waste_2017_kg,
    waste_generated = total_waste_generated_kg
  ) |> 
  group_by(
    year = 2017,
    name_of_company,
  ...
  summarise(
    total_waste_2017         = sum(total_waste_2017, na.rm = TRUE),
    waste_stored_start_year  = sum(waste_stored_start_year, na.rm = TRUE),
    waste_generated    = sum(waste_generated, na.rm = TRUE),
...
waste_generation_17.sheet2 <- read_xlsx(waste_generation_17.path, skip = 2, sheet = 2)

waste_generation_17.by_codes2 <- 
  waste_generation_17.sheet2 |> 
  rename(
    name_of_company        = `NAZIV IN NASLOV`,
    statistical_region     = `STATISTIČNA REGIJA`,
    waste_code             = `ŠT. ODPADKA`,
  ...
  mutate(
    management = recode(
      management,
      "1-lastna obdelava (OVD za obdelavo lastnih odpadkov)" = "waste_treated_by_producer",
      "2-oddano drugemu prevzemniku v RS"               = "waste_handed_to_others_RS",
      "3-poslano v drugo državo članico EU"             = "waste_sent_to_EU"
    ),
  ...
  group_by(
    year, name_of_company, statistical_region,
    waste_code, type_of_waste, waste_category, management
  ) |> 
  summarise(
    total_waste_given_away = sum(total_waste_given_away, na.rm = TRUE),
    .groups = "drop"
  ) |> 
  # pivot so that the management values become column names verbatim
  pivot_wider(
    names_from   = management,
    values_from  = total_waste_given_away,
    values_fill  = 0
  ) |> 
  # roll back up to category-level totals
  group_by(
    year, name_of_company, statistical_region,
    waste_code, type_of_waste, waste_category
  ) |> 
  summarise(
    waste_treated_by_producer    = sum(waste_treated_by_producer,    na.rm = TRUE),
    waste_handed_to_others_RS    = sum(waste_handed_to_others_RS,    na.rm = TRUE),
    waste_sent_to_EU             = sum(waste_sent_to_EU,             na.rm = TRUE),
    total_waste_given_away       = (waste_treated_by_producer
...
waste_collection_17.sheet1 <- read_xlsx(waste_collection_17.path, skip = 1, sheet = 1)
...
waste_collection_17.by_codes1 <- waste_collection_17.sheet1 |>
  ...
  process_waste_column(waste_code, type_of_waste) |>
  mutate(across(everything(), ~ replace_na(.x, 0))) |>
  group_by(year = 2017, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    waste_stored_start_year = sum(waste_stored_start_year) / 1000,
    waste_stored_end_year = sum(waste_stored_end_year) / 1000,
    .groups = "drop"
...
waste_collection_17.sheet2 <- read_xlsx(waste_collection_17.path, skip = 1, sheet = 2)
...
waste_collection_17.by_codes2 <- waste_collection_17.sheet2 |>
  ...
  group_by(year = 2017, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, collected_from) |>
  summarise(
...
waste_collection_17.sheet3 <- read_xlsx(waste_collection_17.path, skip = 1, sheet = 3)
...
waste_collection_17.by_codes3 <- waste_collection_17.sheet3 |>
  ...
  group_by(year = 2017, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, given_to) |>
  summarise(
...
```{r waste_recovery_disposal_storage}
waste_recovery_disposal_17.sheet1 <- read_xlsx(waste_recovery_disposal_17.path, skip = 1, sheet = 1)
...
waste_recovery_disposal_17.by_codes1 <- waste_recovery_disposal_17.sheet1 |>
  ...
  mutate(
    year = 2017,
    across(
      # or across(where(is.numeric)) to only affect numeric cols
      everything(), 
      ~ replace_na(.x, 0)
    )
  ) |> 
...
```{r waste_recovery_disposal_collected}
waste_recovery_disposal_17.sheet2 <- read_xlsx(waste_recovery_disposal_17.path, skip = 1, sheet = 2)
...
waste_recovery_disposal_17.by_codes2 <- waste_recovery_disposal_17.sheet2 |>
  ...
  mutate(
    year = 2017,
    received_from = recode(
      received_from,
      "1-lastni odpadki, samo v primeru OVD za obdelavo lastnih odpadkov"  = "own_waste_OVD_case",
      "2-drug izvirni povzročitelj v RS"                                    = "other_original_producer_RS",
      "3-drug zbiralec v RS"                                                = "other_collector_RS",
      "4-drug izvajalec obdelave v RS"                                      = "other_processing_operator_RS",
      "5-druga država članica EU"                                           = "other_EU_country",
      "6-tretja država (izven EU)"                                          = "other_country_outside_EU"
    ),
    total_waste_received = total_waste_received / 1000
  ) |>
  group_by(year, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, received_from) |>
  summarise(total_waste_received = sum(total_waste_received), .groups = "drop") |>
  pivot_wider(
    names_from = received_from, 
    values_from = total_waste_received, 
    values_fill = 0,
    names_prefix = "waste_received_"
  ) |>
  rowwise() |>
  mutate(
    total_waste_received = sum(c_across(starts_with("waste_received_")), na.rm = TRUE)
  ) |>
  ungroup()
```

```{r treatment_of_waste}
waste_recovery_disposal_17.sheet3 <- read_xlsx(waste_recovery_disposal_17.path, skip = 1, sheet = 3)
...
waste_recovery_disposal_17.by_codes3 <- waste_recovery_disposal_17.sheet3 |>
  ...
  group_by(year, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, treatment_process, treatment_according_to_8th_article) |>
  summarise(
    total_waste_given_away = sum(total_waste_given_away) / 1000, # for tons
    .groups = "drop"
  ) 
...
waste_recovery_disposal_17.sheet4 <- read_xlsx(waste_recovery_disposal_17.path, skip = 1, sheet = 4)
...
waste_recovery_disposal_17.by_codes4 <- waste_recovery_disposal_17.sheet4 |>
  ...
  mutate(
    given_to = recode(
      given_to,
      "1-drug zbiralec v RS"          = "other collector in RS",
      "2-drug izvajalec obdelave v RS" = "other processing operator in RS",
      "3-druga država članica EU"     = "other EU country",
      "4-tretja država (izven EU)"    = "other country (outside EU)"
    ),
  ...
  summarise(
    ...
    waste_generated_collector_RS           = sum(if_else(given_to == "other collector in RS",           total_waste_generated, 0)),
    waste_generated_processing_operator_RS = sum(if_else(given_to == "other processing operator in RS", total_waste_generated, 0)),
    waste_generated_other_EU_country       = sum(if_else(given_to == "other EU country",             total_waste_generated, 0)),
    waste_generated_other_country_outside_EU = sum(if_else(given_to == "other country (outside EU)", total_waste_generated, 0)),
    .groups = "drop"
  )
```
...
waste_recovery_disposal_17.sheet5 <- read_xlsx(waste_recovery_disposal_17.path, skip = 1, sheet = 5)
...
waste_recovery_disposal_17.by_codes5 <- waste_recovery_disposal_17.sheet5 |>
  ...
  filter_codes() |>
  process_waste_column(waste_code, type_of_waste) |> 
  group_by(year = 2017, name_of_company, waste_code, type_of_waste, waste_category) |>
  summarise(
...
waste_recovery_disposal_17.sheet6 <- read_xlsx(waste_recovery_disposal_17.path, skip = 1, sheet = 6)
...
waste_recovery_disposal_17.sheet7 <- read_xlsx(waste_recovery_disposal_17.path, skip = 1, sheet = 7)
...
waste_recovery_disposal_17.by_codes7 <- waste_recovery_disposal_17.sheet7 |>
  ...
  group_by(year, name_of_company, landfill, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_inert_waste_received = sum(total_inert_waste_received) / 1000, # for tons
    total_non_hazardous_waste_received = sum(total_non_hazardous_waste_received) / 1000,
    total_hazardous_waste_received = sum(total_hazardous_waste_received) / 1000,
    total_waste_received_no_estimate = sum(total_waste_received_no_estimate) / 1000,
    total_waste_received_EU = sum(total_waste_received_EU) / 1000,
    total_waste_received_outside_EU = sum(total_waste_received_outside_EU) / 1000,
    total_deposited = sum(total_deposited) / 1000,
    for_construction = sum(for_construction) / 1000,
    for_daily_covering = sum(for_daily_covering) / 1000,
    stored_on_landfill = sum(stored_on_landfill) / 1000,
    rejected = sum(rejected) / 1000,
    .groups = "drop"
  )
```
...
waste_generation_16.sheet1 <- read_xls(waste_generation_16.path, skip = 2, sheet = 1)
...
waste_generation_16.by_codes1 <- waste_generation_16.sheet1 |>
  ...
  # Clean statistical region names
  mutate(
    statistical_region = statistical_region |> 
      (\(x) gsub("[0-9]", "", x))() |> 
      (\(x) gsub(" ", "", x, fixed = TRUE))()
  ) |>
  group_by(year = 2016, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(c(total_waste_generated, waste_stored_start_year, 
            waste_generated, waste_stored_end_year),
           ~sum(.) / 1000),
    .groups = "drop"
  ) |>
  select(year, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, total_waste_generated, waste_stored_start_year, waste_generated, waste_stored_end_year)
```
...
waste_generation_16.sheet2 <- read_xls(waste_generation_16.path, skip = 2, sheet = 2)
...
waste_generation_16.by_codes2 <- waste_generation_16.sheet2 |>
  ...
  group_by(year = 2016, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(c(total_waste_given_away, waste_handed_to_others_RS, waste_sent_to_EU, waste_treated_by_producer),
           ~sum(.) / 1000),
    .groups = "drop"
  )
```
...
```{r waste_storage}
waste_collection_16.sheet1 <- read_xls(waste_collection_16.path, skip = 1, sheet = 1)
...
```{r waste_collected}
waste_collection_16.sheet2 <- read_xls(waste_collection_16.path, skip = 1, sheet = 2)
...
waste_collection_16.by_codes2 <- waste_collection_16.sheet2 |>
  ...
  mutate(
    ...
    # Create all destination columns in one mutate
    waste_from_producers_no_record = ifelse(collected_from == "received from producers without a list", total_waste_collected, 0),
    waste_from_producers_with_record = ifelse(collected_from == "received from producers with a list", total_waste_collected, 0),
    waste_from_collectors_RS = ifelse(collected_from == "received from other collectors", total_waste_collected, 0),
    waste_from_processors_RS = ifelse(collected_from == "received from processing operators", total_waste_collected, 0)
  ...
  group_by(year = 2016, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(c(total_waste_collected, starts_with("waste_from_")), 
           ~ sum(.x, na.rm = TRUE) / 1000  # Convert kg to tons
    ),
    .groups = "drop"
  )
```
...
```{r management_waste_collected}
waste_collection_16.sheet3 <- read_xls(waste_collection_16.path, skip = 1, sheet = 3)
...
waste_collection_16.by_codes3 <- waste_collection_16.sheet3 |>
  ...
  mutate(
    given_to = case_when(
      ...
      TRUE ~ given_to  # Keep original value if no match
    ),
    statistical_region = if_else(is.na(statistical_region), "NEOPREDELJENO", statistical_region),
    # Create destination columns directly
    waste_given_to_processing_operator_RS = ifelse(given_to == "processing operator in RS", total_waste_given_away, 0),
    waste_given_to_other_collector_RS = ifelse(given_to == "other collector in RS", total_waste_given_away, 0),
    waste_given_to_other_EU_country = ifelse(given_to == "other EU country", total_waste_given_away, 0)
  ) |> 
  group_by(year = 2016, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_waste_given_away = sum(total_waste_given_away, na.rm = TRUE) / 1000,
    across(starts_with("waste_given_to"), ~ sum(.x, na.rm = TRUE) / 1000),
    .groups = "drop"
  )
```
...
```{r waste_storage}
waste_recovery_disposal_16.sheet1 <- read_xls(waste_recovery_disposal_16.path, skip = 1, sheet = 1)
...
waste_recovery_disposal_16.by_codes1 <- waste_recovery_disposal_16.sheet1 |>
  ...
  mutate(
    waste_stored_change = waste_stored_end_year - waste_stored_start_year
  )

```

```{r waste_collected}
waste_recovery_disposal_16.sheet2 <- read_xls(waste_recovery_disposal_16.path, skip = 1, sheet = 2)
...
waste_recovery_disposal_16.by_codes2 <- waste_recovery_disposal_16.sheet2 |>
  ...
  mutate(
    ...
    waste_collected_from_collector_RS = ifelse(collected_from == "collector in RS", total_waste_collected, 0),
    waste_collected_from_other_original_producer_RS = ifelse(collected_from == "other original producer in RS", total_waste_collected, 0),
    waste_collected_from_other_processing_operator_RS = ifelse(collected_from == "other processing operator in RS", total_waste_collected, 0),
    waste_collected_from_other_EU_country = ifelse(collected_from == "other EU country", total_waste_collected, 0),
    waste_collected_from_own_waste = ifelse(collected_from == "own waste", total_waste_collected, 0),
    waste_collected_from_other_country_outside_EU = ifelse(collected_from == "other country (outside EU)", total_waste_collected, 0)
  ) |> 
  group_by(year = 2016, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(c(total_waste_collected, starts_with("waste_collected_from")), 
    ~ sum(.x, na.rm = TRUE) / 1000  # Convert kg to tons
    ),
    .groups = "drop"
  )
...
waste_recovery_disposal_16.sheet3 <- read_xls(waste_recovery_disposal_16.path, skip = 1, sheet = 3)
...
waste_recovery_disposal_16.by_codes3 <- waste_recovery_disposal_16.sheet3 |>
  ...
  group_by(year = 2016, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, treatment_process, treatment_according_to_8th_article) |>
  summarise(
    total_waste_given_away = sum(total_waste_given_away, na.rm = TRUE) / 1000,
    .groups = "drop"
  )
...
waste_recovery_disposal_16.sheet4 <- read_xls(waste_recovery_disposal_16.path, skip = 1, sheet = 4)

waste_recovery_disposal_16.by_codes4 <- 
  waste_recovery_disposal_16.sheet4 |> 
  select(
    name_of_company                     = `NAZIV IN NASLOV`,
    statistical_region                  = `STATISTIČNA REGIJA`,
    waste_code                          = `ŠT. ODPADKA`,
  ...
  mutate(
    ...
    waste_generated_to_processing_operator_RS   = if_else(given_to == "processing operator in RS",   total_waste_generated_untreated, 0),
    waste_generated_to_other_collector_RS       = if_else(given_to == "other collector in RS",       total_waste_generated_untreated, 0),
    waste_generated_to_other_EU_country         = if_else(given_to == "other EU country",            total_waste_generated_untreated, 0),
    waste_generated_to_other_country_outside_EU = if_else(given_to == "other country (outside EU)", total_waste_generated_untreated, 0)
  ) |> 
  group_by(
    year = 2016,
    name_of_company,
  ...
  summarise(
    total_waste_generated_untreated     = sum(total_waste_generated_untreated,     na.rm = TRUE) / 1000,
    waste_generated_to_processing_operator_RS   = sum(waste_generated_to_processing_operator_RS,   na.rm = TRUE) / 1000,
    waste_generated_to_other_collector_RS       = sum(waste_generated_to_other_collector_RS,       na.rm = TRUE) / 1000,
    waste_generated_to_other_EU_country         = sum(waste_generated_to_other_EU_country,         na.rm = TRUE) / 1000,
    waste_generated_to_other_country_outside_EU = sum(waste_generated_to_other_country_outside_EU, na.rm = TRUE) / 1000,
    .groups = "drop"
  )
```
