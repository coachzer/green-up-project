...
data18.sheet2 <- data18.sheet2 |>
  ...
  rename_region() |>
  mutate(
    waste_self_processed            = if_else(delivered_to == "self_processed",             total_waste_given_away, 0),
    waste_to_operator_RS            = if_else(delivered_to == "processing_operator_RS",    total_waste_given_away, 0),
    waste_to_other_EU_country       = if_else(delivered_to == "other_EU_country",          total_waste_given_away, 0),
    waste_to_other_country_outside_EU = if_else(delivered_to == "other_country_outside_EU", total_waste_given_away, 0)
  ) |>
  group_by(
    year, name_of_company, statistical_region,
    waste_code, type_of_waste, waste_category
  ...
  summarise(
    total_waste_given_away           = sum(total_waste_given_away)           / 1000,
    waste_self_processed             = sum(waste_self_processed)             / 1000,
    waste_to_operator_RS             = sum(waste_to_operator_RS)             / 1000,
    waste_to_other_EU_country        = sum(waste_to_other_EU_country)        / 1000,
    waste_to_other_country_outside_EU = sum(waste_to_other_country_outside_EU)/ 1000,
    .groups = "drop"
  )
```
...
waste_generation_17.by_codes1 <- waste_generation_17.sheet1 |>
  ...
  mutate(
    ...
    total_waste_generated = total_waste_generated_kg
  ) |> 
  group_by(
    year = 2017,
    name_of_company,
  ...
  summarise(
    ...
    total_waste_generated    = sum(total_waste_generated, na.rm = TRUE),
    waste_stored_end_year    = sum(waste_stored_end_year, na.rm = TRUE),
    waste_inventory_change   = waste_stored_end_year - waste_stored_start_year,
    .groups = "drop"
  )
```
...
waste_generation_17.by_codes2 <- waste_generation_17.sheet2 |> 
  ...
  mutate(
    management = recode(
      management,
      "1-lastna obdelava (OVD za obdelavo lastnih odpadkov)" = "own_processing",
      "2-oddano drugemu prevzemniku v RS"               = "to_collector_RS",
      "3-poslano v drugo državo članico EU"             = "to_EU_country"
    ),
    total_tonnes = total_waste_given_away / 1000,
    year = 2017
  ) |>
  group_by(year, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, management) |>
  summarise(
    total_tonnes = sum(total_tonnes, na.rm = TRUE),
    .groups = "drop"
  ) |>
  pivot_wider(
    names_from   = management,
    values_from  = total_tonnes,
    names_prefix = "waste_given_to_",
    values_fill  = 0
  ) |>
  group_by(year, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_waste_given_away    = sum(waste_given_to_own_processing, na.rm = TRUE)
                              + sum(waste_given_to_to_collector_RS, na.rm = TRUE)
                              + sum(waste_given_to_to_EU_country, na.rm = TRUE),
    waste_given_to_own_processing  = sum(waste_given_to_own_processing, na.rm = TRUE),
    waste_given_to_to_collector_RS = sum(waste_given_to_to_collector_RS, na.rm = TRUE),
    waste_given_to_to_EU_country   = sum(waste_given_to_to_EU_country, na.rm = TRUE),
    .groups = "drop"
  )
```
...
waste_generation_16.by_codes1 <- waste_generation_16.sheet1 |>
  ...
  group_by(name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
...
waste_generation_16.by_codes2 <- waste_generation_16.sheet2 |>
  ...
  group_by(name_of_company, statistical_region, waste_code, type_of_waste, waste_category, management) |>
  summarise(
    across(c(total_waste_given_away),
           ~sum(.) / 1000),
    .groups = "drop"
  ) |>
  mutate(year = 2016) |> 
  select(year, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, total_waste_given_away, management)
```
