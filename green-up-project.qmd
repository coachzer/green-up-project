...
filter_df_by_codes <- function(df) {
  ...
  if (any(columns_to_rename %in% colnames(df))) {
    df <- df |>
      rename_with( ~ ifelse(. %in% columns_to_rename, "waste_code", .), cols = columns_to_rename) |>
      filter(
        waste_code %in%
          codes |
  ...
  if ("Odpadki na izhodu iz postopka obdelave in oddani neobdelani odpadki številka" %in% colnames(df)) {
    df <- df |>
      rename(treatment_outlet_delivered_waste_code = `Odpadki na izhodu iz postopka obdelave in oddani neobdelani odpadki številka`)
    
    df <- df |>
      filter(
        treatment_outlet_delivered_waste_code %in%
          codes |
