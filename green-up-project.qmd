...
# Table 1a: Waste collected by collection centre, 2023
collected_centre.23 <- processed_sheets[[2]] |> select(-Poročilo)

collected_centre.23 <- collected_centre.23 |>
  rename(
    year = `Leto`,
    statistical_region = `Statistična regija`,
    collection_centre = `Zbirni center`,
    type_of_waste = `Naziv odpadka`,
    waste_collected_by_centre = `Zbrani odpadki`
  )

collected_centre.23 <- rename_waste_names(collected_centre.23)
...
processed_sheets[[2]] <- collected_centre.23 
...
collected_municipality.23 <- collected_municipality.23 |>
  rename(
    ...
    type_of_waste = `Naziv odpadka`,
    waste_collected_public_service = `Zbrani v okviru javne službe zbiranja komunalnih odpadkov`,
    total_waste_collected = `Komunalni odpadki in odpadek 16 01 03 po občini izvora kg`,
...
collected_municipality.23 <- collected_municipality.23 |>
  group_by(year, statistical_region, name_of_municipality, type_of_waste, waste_collected_public_service) |>
  summarise(total_waste_collected = sum(total_waste_collected) / 1000) |>
  ungroup()
...
collected_management.23 <- collected_management.23 |>
  rename(
    ...
    statistical_region = `Statistična regija`,
    type_of_waste = `Naziv odpadka`,
    waste_collected_from_collectors_RS = `Odpadki, oddani v nadaljnje ravnanje zbiralcem v RS kg`,
    waste_collected_from_processors_RS = `Odpadki, oddani v nadaljnje ravnanje izvajalcem obdelave v RS kg`,
    waste_collected_to_EU = `Odpadki, oddani v nadaljnje ravnanje v druge države članice EU kg`,
    waste_collected_outside_EU = `Odpadki, oddani v nadaljnje ravnanje v države izven EU kg`,
    # or delivered to the collector's warehouse
    waste_collected_to_collectors_warehouse = `Odpadki oddani v skladišče zbiralca (stanje na dan 31.12.)`
  )
...
collected_management.23 <- collected_management.23 |>
  group_by(year, statistical_region, type_of_waste) |>
  summarise(waste_collected_from_collectors_RS = sum(waste_collected_from_collectors_RS) / 1000,
            waste_collected_from_processors_RS = sum(waste_collected_from_processors_RS) / 1000,
            waste_collected_to_EU = sum(waste_collected_to_EU) / 1000,
            waste_collected_outside_EU = sum(waste_collected_outside_EU) / 1000,
...
treatment.23 <- treatment.23 |>
  rename(
    ...
    stored_quantity_start_year = `Skladiščena količina na dan 1.1.2023 kg`
  )
...
treatment.23 <- treatment.23 |>
  group_by(year, statistical_region, type_of_waste_inlet, type_of_waste_outlet, `V postopku obdelave nastala snov ali predmet`) |>
  summarise(stored_quantity_start_year = sum(stored_quantity_start_year) / 1000) |>
  ungroup()
...
treatment_received.23 <- treatment_received.23 |>
  rename(
    ...
    statistical_region = `Statistična regija`,
    type_of_waste = `Naziv odpadka`,
    stored_quantity_start_year = `Prevzeti odpadki neobdelani odpadki iz skladišča (stanje na dan 1.1.) kg`,
    own_waste_received = `Prevzeti odpadki lastni odpadki kg`,
    # generators
    ...
    waste_received_from_outside_EU = `Prevzeti odpadki iz držav izven EU kg`,
...
treatment_received.23 <- treatment_received.23 |>
  group_by(year, statistical_region, type_of_waste) |>
  summarise(stored_quantity_start_year = sum(stored_quantity_start_year) / 1000,
            own_waste_received = sum(own_waste_received) / 1000,
            ...
            waste_received_from_outside_EU = sum(waste_received_from_outside_EU) / 1000,
...
management.23 <- management.23 |>
  ...
  summarise(waste_sent_to_self_storage_end_year = sum(waste_sent_to_self_storage_end_year) / 1000,
...
management_products.23 <- management_products.23 |>
  ...
  summarise(storage_operator_end_year = sum(storage_operator_end_year) / 1000,
...
processed_sheets[[2]] <- processed_sheets[[2]] |> 
  group_by(year, statistical_region, type_of_waste) |>
  summarise(total_collected_waste = sum(total_collected_waste) / 1000,
            waste_from_producers_no_record = sum(waste_from_producers_no_record) / 1000,
            waste_from_producers_with_record = sum(waste_from_producers_with_record) / 1000,
            waste_from_collectors_RS = sum(waste_from_collectors_RS) / 1000,
            waste_from_processors_RS = sum(waste_from_processors_RS) / 1000) |>
...
waste_recovery_disposal_17.by_codes7 <- waste_recovery_disposal_17.by_codes7 |>
  ...
  summarise(total_non_hazardous_waste_received = sum(total_non_hazardous_waste_received) / 1000, # for tons
