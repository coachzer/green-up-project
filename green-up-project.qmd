...
processed_sheets[[5]] <- management.22 |> select(year, statistical_region, waste_code, type_of_waste, waste_category, total_waste_given_away, waste_handed_to_collectors_RS, waste_delivered_to_operators_RS, waste_sent_to_EU, waste_sent_to_non_EU)
...
pre_collection.21 <- categorize_waste(pre_collection.21, waste_code_col = "waste_code")
...
collected.21 <- categorize_waste(collected.21, waste_code_col = "waste_code")
...
wc_by_origin <- categorize_waste(wc_by_origin, waste_code_col = "waste_code")
...
management.21 <- categorize_waste(management.21, waste_code_col = "waste_code")
...
waste_storage.21 <- categorize_waste(waste_storage.21, waste_code_col = "waste_code")
...
waste_received.21 <- categorize_waste(waste_received.21, waste_code_col = "waste_code")
...
treating_waste.21 <- categorize_waste(treating_waste.21, waste_code_col = "waste_code")
...
waste_treatment.21 <- categorize_waste(waste_treatment.21, waste_code_col = "waste_code")
...
landfill_waste.21 <- categorize_waste(landfill_waste.21, waste_code_col = "waste_code")
...
generation.20 <- categorize_waste(generation.20, waste_code_col = "waste_code")
...
collected_waste.20 <- processed_sheets[[2]] |>
  ...
  # 2) categorize
  process_waste_column(waste_code, type_of_waste) |>
  # 3+4) convert to tonnes & compute total
  mutate(
    across(
      c(
...
municipal_waste_origin.20 <- processed_sheets[[4]] |>
  ...
  # 2) Clean up waste_code/type and categorize
  process_waste_column(waste_code, type_of_waste) |>
  # 3) Add the statistical_region
  left_join(municipality_region_mapping, by = "name_of_municipality") |>
  # 4) Convert from kg to tonnes
  mutate(
    municipal_waste_collected_by_municipality = municipal_waste_collected_by_municipality / 1000
  ) |>
  # 5) Keep only the final columns in the desired order
  select(
    year,
    statistical_region,
    name_of_municipality,
...
management_collected_waste.20 <- processed_sheets[[5]] |>
  ...
  process_waste_column(waste_code, type_of_waste) |>
  # 1) divide all four “kg”‐columns by 1,000
  # 2) compute the total as the row‐sum of those
  mutate(
    across(
  ...
  # finally, keep only the columns you care about
  select(
...
home_composting.20 <- processed_sheets[[6]] |>
  # 1) Pick & rename only the columns you care about
  select(
    year                           = `Leto 2020`,
...
waste_storage.20 <- processed_sheets[[1]] |>
  ...
  # 2) categorize based on your code/type
  process_waste_column(waste_code, type_of_waste) |>
  # 3+4) group & summarise both columns at once
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(
      c(waste_stored_start_year, waste_stored_end_year),
...
waste_received.20 <- processed_sheets[[2]] |>
  # 1) pick & rename only the cols you need
  select(
    year                             = Leto,
    waste_code,
    statistical_region               = `Statistična regija`,
  ...
  # 2) categorize code & type
  process_waste_column(waste_code, type_of_waste) |>
  # 3) group & summarise all kg‐columns (summing then converting to tonnes)
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(
      c(
...
treating_waste.20 <- processed_sheets[[3]] |>
  ...
  # 2) derive waste_category, etc.
  process_waste_column(waste_code, type_of_waste) |>
  # 3+4) group & summarise that one column into tonnes
  group_by(year, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    across(
...
management_waste.20 <- processed_sheets[[4]] |>
  # 1) pick & rename exactly what you need
  select(
    year                              = Leto,
    statistical_region                = `Statistična regija`,
  ...
  # 2) categorize inlet & outlet
  process_waste_column(treatment_inlet_delivered_waste_code, input_waste_name) |>
  process_waste_column(treatment_outlet_delivered_waste_code, output_waste_name) |>
  # 3+4) aggregate all four kg‐columns (sum & convert to tonnes)
  group_by(year, statistical_region, treatment_inlet_delivered_waste_code, input_waste_name, waste_category, treatment_outlet_delivered_waste_code, output_waste_name, waste_category_1) |>
  summarise(
    across(
      c(
...
management_waste.20a <- processed_sheets[[5]] |>
  ...
  # 2) categorize input code/name
  process_waste_column(input_waste_code, input_waste_name) |>
  # 3) recode the output names
  mutate(
...
waste_treatment.20 <- processed_sheets[[6]] |>
  ...
  # 3) attach region
  left_join(municipality_region_mapping, by = "name_of_municipality") |>
  # 4) sum & convert to tonnes in one go
  group_by(year, statistical_region, name_of_municipality, waste_code, type_of_waste, waste_category) |>
  summarise(
...
waste_storage.19 <- waste_storage.19 |>
  group_by(statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    waste_stored_start_year = sum(waste_stored_start_year) / 1000, # convert to tons
    waste_stored_end_year = sum(waste_stored_end_year) / 1000
  ) |>
  ungroup() |>
  mutate(year = 2019)
...
waste_storage.19 <- waste_storage.19 |>
  select(year, statistical_region, waste_code, type_of_waste, waste_category, waste_stored_start_year, waste_stored_end_year)
...
waste_collected.19 <- waste_collected.19 |>
  group_by(statistical_region, waste_code, type_of_waste, waste_category, gathered_from) |>
  summarise(
...
waste_collected.19 <- waste_collected.19 |>
  select(year, statistical_region, waste_code, type_of_waste, waste_category, gathered_from, waste_received, waste_from_producers_no_record, waste_from_producers_with_record, waste_from_collectors_RS, waste_from_processors_RS)
...
waste_collection_19.sheet3 <- waste_collected_municipality.19 |> 
  select(year, name_of_company, waste_code, type_of_waste, waste_category, total_waste_collected, municipal_waste_collected, waste_collected_from_activities)
...
waste_generation_16.by_codes1 <- waste_generation_16.by_codes1 |>
  select(year, statistical_region, waste_code, type_of_waste, waste_category, total_waste_generated, waste_stored_start_year, waste_generated, waste_stored_end_year)
```
