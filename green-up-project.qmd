...
waste_treated.23 <- waste_treated.23 |>
  ...
  group_by(year, statistical_region, treatment_inlet_waste_code, type_of_waste, waste_category,  treatment_code, description_of_treatment) |>
  summarise(
...
waste_treatment.19 <- waste_recovery_19.sheet3 |>
  ...
  group_by(year = 2019, name_of_company, statistical_region, type_of_waste, treatment_process, treatment_after_8) |>
  summarise(
...
waste_recovery_19.sheet6 <- waste_difference.19 |>
  select(year, name_of_company, waste_code, type_of_waste, waste_category, lost_mass)
```
...
waste_recovery_19.sheet8 <- waste_volumes_landfill.19 |> select(year, landfill, waste_code, type_of_waste, waste_category, non_hazardous_waste_received, disposed)
```
...
waste_recovery_disposal_17.by_codes2 <- waste_recovery_disposal_17.sheet2 |>
  ...
  mutate(
    ...
    received_from = recode(
      received_from,
      "1-lastni odpadki, samo v primeru OVD za obdelavo lastnih odpadkov"  = "own waste, OVD case",
      "2-drug izvirni povzročitelj v RS"                                    = "other original producer in RS",
      "3-drug zbiralec v RS"                                                = "other collector in RS",
      "4-drug izvajalec obdelave v RS"                                      = "other processing operator in RS",
      "5-druga država članica EU"                                           = "other EU country",
      "6-tretja država (izven EU)"                                          = "other country (outside EU)"
    ),
    total_waste_received = total_waste_received / 1000
  ) |>
  # group once & compute totals + buckets
  group_by(year, name_of_company, statistical_region, waste_code, type_of_waste, waste_category) |>
  summarise(
    total_waste_received                   = sum(total_waste_received),
    waste_received_own_waste               = sum(if_else(received_from == "own waste, OVD case",               total_waste_received, 0)),
    waste_received_other_producer_RS       = sum(if_else(received_from == "other original producer in RS", total_waste_received, 0)),
    waste_received_collector_RS            = sum(if_else(received_from == "other collector in RS",       total_waste_received, 0)),
    waste_received_processing_operator_RS  = sum(if_else(received_from == "other processing operator in RS", total_waste_received, 0)),
    waste_received_other_EU_country        = sum(if_else(received_from == "other EU country",           total_waste_received, 0)),
    waste_received_other_country_outside_EU = sum(if_else(received_from == "other country (outside EU)", total_waste_received, 0)),
    .groups = "drop"
  )
```
...
waste_recovery_disposal_16.by_codes3 <- waste_recovery_disposal_16.sheet3 |>
  ...
  filter_codes() |> 
  categorize_waste() |> 
  group_by(year = 2016, name_of_company, statistical_region, waste_code, type_of_waste, waste_category, treatment_process) |>
  summarise(
    total_waste_given_away = sum(total_waste_given_away, na.rm = TRUE) / 1000,
    .groups = "drop"
  )
